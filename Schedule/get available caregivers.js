...
function weeklyDistributionCheck(caregiverName, candidateShiftHours, employeesData, scheduleData, scheduleStartDate) {
    ...
    if (Array.isArray(scheduleData)) {
        ...
        scheduleData.forEach(schedule => {
            ...
            // ✅ FIXED: Exclude ghost shifts from weekly hours calculation ONLY
            // Check BOTH Shift_Status and Scheduling_Status fields for ghost shifts
            const shiftStatus = normStr(schedule.Shift_Status || '').toLowerCase();
            const schedulingStatus = normStr(schedule.Scheduling_Status || '').toLowerCase();

            if (shiftStatus.indexOf('ghost shift') !== -1 || schedulingStatus.indexOf('ghost shift') !== -1) {
                console.log(`Excluding ghost shift from weekly hours for ${caregiverName} on ${scheduleDate} (Shift_Status: ${shiftStatus}, Scheduling_Status: ${schedulingStatus})`);
                return;
            }
...
function fetchCaregivers() {
    function onCaregiverResponse(response) {
        ...
        console.log("Processed CaregiverList:", CaregiverList);
    }

    app_lib.getTxnUsingIncFields(null, fieldsJson, onCaregiverResponse, null, "SVC_TYPE_3");
}

// ===== Convert time to minutes (FIXED: handles both "4:00" and "04:00" formats) =====
function toMinutes(t) {
    if (!t) return 0;
    var cleanTime = String(t).trim();
    var [h, m] = cleanTime.split(":").map(Number);
...
// ===== NEW: Helper function to check time overlap =====
function timeOverlap(start1, end1, start2, end2) {
    return start1 < end2 && start2 < end1;
}

// ===== NEW: Convert day name to key format =====
...
// ===== NEW: Custom Time Availability Check =====
function checkCustomTimeAvailability(caregiverRecordId, tableId, dayName, startMin, endMin, callback) {
    ...
    if (!caregiverRecordId || !tableId) {
        ...
        callback({ available: false, reason: "Missing availability details" });
        return;
    }
    ...
    app_lib.rowsOf(caregiverRecordId, tableId, function (rows) {
        ...
        if (!Array.isArray(rows) || rows.length === 0) {
            ...
            callback({ available: false, reason: "No custom availability defined" });
            return;
        }
...
// ===== NEW: Enhanced availability check supporting all types =====
function checkCaregiverAvailability(caregiverRecord, dayName, startMin, endMin, callback) {
    ...
    if (!caregiverRecord || !caregiverRecord.fields) {
        callback({ available: false, reason: "Missing caregiver record" });
        return;
    }
    ...
    for (var i = 0; i < segments.length; i++) {
        ...
        if (availability !== 'yes' && availability !== 'true') {
...
// ===== Validate selected caregiver (ENHANCED with Custom Time support) =====
function validateSelectedCaregiver() {
    // Corrected line: Convert the value to a string before trimming
    ...
    var caregiverSchedules = CaregiverList.filter(c => String(c.Caregiver_Id) === selectedId);
    ...
    // ===== Conflict check =====
    var conflicts = caregiverSchedules.filter(s => {
        if (normalizeDate(s.Schedule_Start_Date) !== normalizeDate(inputDate)) return false;

        var sStart = toMinutes(s.Schedule_Start_Time);
        var sEnd = toMinutes(s.Schedule_End_Time);

        return (inputStart < sEnd && inputEnd > sStart);
    });
    ...
    if (conflicts.length > 0) {
        ...
        conflicts.forEach((c, idx) => {
            conflictMsg +=
                (idx + 1) + ". " + c.Schedule_Start_Time + " – " + c.Schedule_End_Time +
