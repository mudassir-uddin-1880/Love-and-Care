...
function assignCaregiverToSchedule(clientData, dayObj, schedule, employeesDetails,
    ...
    for (var ac = 0; ac < allNames.length; ac++) {
        ...
        if (!isAvailableForTimeSlot || isCaregiverOnLeaveNow) {
            // Fallback to other caregivers
            res.debugReasons.push({
                priority: 'Primary Caregiver Unavailable',
                allowed: false,
                reason: 'Primary caregiver is unavailable. Proceeding with other caregivers.'
            });
        }
        if (primaryCaregiverName && normName(cand) === normName(primaryCaregiverName)) {
            res.debugReasons.push({
                priority: 'Primary Caregiver Evaluation',
                allowed: true,
                reason: 'Primary caregiver evaluated with priority boost.'
            });
        }
        if (primaryCaregiverName && normName(cand) === normName(primaryCaregiverName)) {
            candidateResult.priorityScore += 10; // Boost priority for primary caregiver
            candidateResult.isPrimary = true;
        }
        if (primaryCaregiverName && normName(cand) === normName(primaryCaregiverName)) {
            res.primaryCaregiverChecked = true;
        }
        var isActive = isCaregiverActive(cand, employeesDetails);
        if (!isActive) {
