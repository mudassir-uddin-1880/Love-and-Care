...
var ghostShifts = [];
var globalCaregiverTimeSlotsSet = {};

// After loading employeesDetails:
var employeeRecordMap = {};
for (var i = 0; i < employeesDetails.length; i++) {
    var emp = employeesDetails[i];
    var name = normName(safeGetValue(emp, 'fields.Employee_Full_Name.value', ''));
    if (name) employeeRecordMap[name] = emp;
}

// After loading allClientsScheduleData:
var clientRecordMap = {};
for (var i = 0; i < allClientsScheduleData.length; i++) {
    var client = allClientsScheduleData[i];
    if (client && client.id) clientRecordMap[client.id] = client;
}

// Add after loading actualSchedulingData:
var caregiverScheduleMap = {};
var clientScheduleMap = {};
for (var i = 0; i < actualSchedulingData.length; i++) {
    var rec = actualSchedulingData[i];
    if (!rec || !rec.fields) continue;
    var cg = normName(safeGetValue(rec.fields, 'Expected_Caregiver.value', ''));
    var cl = normName(safeGetValue(rec.fields, 'Client_Name.value', ''));
    if (cg) {
        caregiverScheduleMap[cg] = caregiverScheduleMap[cg] || [];
        caregiverScheduleMap[cg].push(rec);
    }
    if (cl) {
        clientScheduleMap[cl] = clientScheduleMap[cl] || [];
        clientScheduleMap[cl].push(rec);
    }
}

// Add historicalCountCache:
var historicalCountCache = {};
for (var i = 0; i < actualSchedulingData.length; i++) {
    var rec = actualSchedulingData[i];
    if (!rec || !rec.fields) continue;
    var f = rec.fields;
    var cgNorm = normName(safeGetValue(f, 'Expected_Caregiver.value', ''));
    var clNorm = normName(safeGetValue(f, 'Client_Name.value', ''));
    var status = safeGetValue(f, 'Scheduling_Status.value', '');
    var sDate = safeGetValue(f, 'Schedule_Start_Date.value', '');
    if (cgNorm && clNorm && sDate &&
        (status === 'Approved' || status === 'Completed' || status === 'Scheduled Completed')) {
        var key = cgNorm + '|' + clNorm;
        if (!historicalCountCache[key]) historicalCountCache[key] = [];
        historicalCountCache[key].push(sDate);
    }
}

// Add weeklyHoursCache:
var weeklyHoursCache = {};
function preCalculateWeeklyHours(actualSchedulingData, currDate) {
    ...
    for (var i = 0; i < actualSchedulingData.length; i++) {
        ...
        var cgNorm = normName(safeGetValue(f, 'Actual_Caregiver.value', ''));
        var hours = safeParseNumber(safeGetValue(f, 'Actual_Hours.value', 0), 0);
        if (cgNorm) {
            weeklyHoursCache[cgNorm] = (weeklyHoursCache[cgNorm] || 0) + hours;
        }
    }
}


preCalculateWeeklyHours(actualSchedulingData, currDate);
...
function getCaregiverWorkedHoursThisWeek(caregiverName, actualSchedulingData, weekStartDate, weekEndDate) {
    ...
    return weeklyHoursCache[cgNorm] || 0;
}
...
function isCaregiverAvailable(caregiverName, targetClientId, targetDate, targetStartTime, targetEndTime) {
    ...
    var slotKey = targetDate + '|' + targetStartTime + '|' + targetEndTime;
    if (globalCaregiverTimeSlotsSet[caregiverName] && globalCaregiverTimeSlotsSet[caregiverName][slotKey]) {
        return false; // Exact match found
    }

    // Fallback to time overlap check only if no exact match
...
function getEmployeeRecordByName(caregiverName, employeesDetails) {
    ...
    return employeeRecordMap[target] || null;
}
...
function getHistoricalCount(caregiverName, clientName, actualSchedulingData, lookbackDays) {
    if (!caregiverName || !clientName) return 0;

    var cgNorm = normName(caregiverName);
    var clientNorm = normName(clientName);
    var key = cgNorm + '|' + clientNorm;

    var dates = historicalCountCache[key];
    ...
    for (var i = 0; i < dates.length; i++) {
        if (dates[i] >= cutoff && dates[i] <= currDate) count++;
    }
    return count;
}
...
var lastWeekCaregiverCache = {};
for (var i = 0; i < allClientsScheduleData.length; i++) {
    var clientData = allClientsScheduleData[i];
    if (clientData && clientData.id) {
        lastWeekCaregiverCache[clientData.id] = getLastWeekCaregiver(clientData, actualSchedulingData);
    }
}

function getLastWeekCaregiverCached(clientData, actualSchedulingData) {
    return lastWeekCaregiverCache[clientData.id] || null;
}
...
function getLastWeekCaregiver(clientData, actualSchedulingData) {
    ...
    for (var i = 0; i < actualSchedulingData.length; i++) {
        ...
        // Use Expected_Hours instead of Actual_Hours
        ...
        if ((status === 'Completed' || status === 'Scheduled Completed') &&
            ...
            // Initialize caregiver tracking - use normalized name as key
...
function calculateClientSpecificHours(caregiverName, clientData, actualSchedulingData) {
    ...
    for (var i = 0; i < actualSchedulingData.length; i++) {
        ...
        var fields = record.fields;
        var recordClientName = safeGetValue(fields, 'Client_Name.value', '');
        var recordCaregiverName = safeGetValue(fields, 'Expected_Caregiver.value', '');
        var status = safeGetValue(fields, 'Scheduling_Status.value', '');
        var hours = safeParseNumber(safeGetValue(fields, 'Expected_Hours.value', 0), 0);

        // Normalize for comparison
        var recordClientNameNorm = normName(recordClientName);
        var recordCaregiverNameNorm = normName(recordCaregiverName);

        if ((status === 'Completed' || status === 'Approved' || status === 'Scheduled Completed') &&
            recordClientNameNorm === clientNameNorm &&
            recordCaregiverNameNorm === caregiverNameNorm) {
            totalHours += hours;
        }
    }
    return totalHours;
...
function evaluateCandidateWithPriorities(candidateName, employeeData, candidateShiftHours, settings, clientData, debugReasons, isPrimary) {
    ...
    var lastWeekCaregiver = getLastWeekCaregiverCached(clientData, actualSchedulingData);
...
function assignCaregiverToSchedule(clientData, dayObj, schedule, employeesDetails,
    ...
    if (lastWeekCompleteSchedule && lastWeekCompleteSchedule[dayObj.day]) {
        ...
        if (matchingAssignment) {
            ...
            // **CRITICAL CHECKS ONLY: Skip non-critical validations for last week caregiver**
            var canCopyAssignment = true;
            var conflictReason = '';

            // 1. Check if caregiver is on leave (CRITICAL)
            ...
            // 2. Check if caregiver is blocked by client (CRITICAL)
            ...
            // 3. Check if caregiver has time conflict (CRITICAL)
            if (canCopyAssignment) {
            ...
            // - Ghost shift check (isGhostCaregiverForDate)
            // - Weekly hours check (weeklyDistributionCheck)
            // - Time slot availability (isCaregiverAvailableForSchedule) - already commented out
            ...
            if (canCopyAssignment) {
                ...
                res.debugReasons.push({
                    ...
                    reason: 'Successfully copied exact schedule from last week: ' + lastWeekCaregiver +
                        ' (passed critical validation checks: leave status, blocklist, time conflicts. Skipped: ghost shifts, weekly hours, mandatory requirements)'
                });
                ...
                if (!globalCaregiverTimeSlots[lastWeekCaregiver]) {
                    ...
                    globalCaregiverTimeSlotsSet[lastWeekCaregiver] = {};
                }
                var slotKey = dayObj.iso + '|' + schedule.startTime + '|' + schedule.endTime;
                ...
                globalCaregiverTimeSlotsSet[lastWeekCaregiver][slotKey] = true;
    ...
    if (lastWeekCaregiver) {
        ...
        // 1. Check if last week caregiver is on leave (CRITICAL)
        ...
        // 3. Check basic time availability (CRITICAL)
        if (canAssignLastWeekCaregiver) {
    ...
    if (availableCaregivers.length > 0) {
        ...
        for (var j = 0; j < actualSchedulingData.length; j++) {
            ...
            if (normName(recClientName) === normName(clientName)) {
                var hours = safeParseNumber(safeGetValue(f, 'Expected_Hours.value', 0), 0);
                totalClientHours += hours;
                var cgName = safeGetValue(f, 'Expected_Caregiver.value', '');
                var cgNameNorm = normName(cgName);
                if (cgNameNorm) {
                    actualWorkedHoursMap[cgNameNorm] = (actualWorkedHoursMap[cgNameNorm] || 0) + hours;
                }
            }
        }
    ...
    if (availableCaregivers.length > 0) {
        ...
        if (!globalCaregiverTimeSlots[bestCandidate.caregiverName]) {
            ...
            globalCaregiverTimeSlotsSet[bestCandidate.caregiverName] = {}; // ADD THIS
        }

        var slotKey = dayObj.iso + '|' + schedule.startTime + '|' + schedule.endTime; // ADD THIS
        ...
        globalCaregiverTimeSlotsSet[bestCandidate.caregiverName][slotKey] = true; // ADD THIS
...
function buildAllClientSchedules(allClientsData, clientSchedulesData, next7Days) {
    ...
    for (var i = 0; i < schedulesList.length; i++) {
        ...
        // SKIP if endTime is before startTime
        ...
        for (var d2 = 0; d2 < next7Days.length; d2++) { // <-- FIXED: use d2++
...
function getClientSchedulesFromAPI(clientId, clientSchedulesData) {
    ...
    for (var i = 0; i < schedulesList.length; i++) {
        ...
        // Allow overnight shifts (e.g., 19:00–7:00)
        schedules.push({
            day: dayName,
            startTime: startTime,
            endTime: endTime,
            ...
            isOvernight: startTime >= endTime // mark overnight
        });
    }
