// Working code With Dynamic Setting changes with leavs Issues resloved also caregiver availaibitylist issue resolved
...
var result = {
    debug: {
        ...
        // ... additional debug info added throughout
    },
    conflicts: {
        total: 0,
        details: []
...
function isPrimaryCaregiverAvailable(primaryName, employeesDetails, leavesData, next7Days) {
    ...
    // Note: This is a general check without specific schedule times
    if (isArray(next7Days) && isArray(leavesData)) {
        for (var d = 0; d < next7Days.length; d++) {
            if (isCaregiverOnLeave(primaryName, next7Days[d].iso, leavesData)) {
                return "No"; // On leave during the period (treating as full day if no time specified)
            }
        }
    }
...
function getPrioritySettings(settingsTableData) {
    ...
    if (!settingsTableData || !settingsTableData.data || !isArray(settingsTableData.data)) {
        return result;
    }
...
// ---------------------------------------------------------------------------
// WEEKLY HOURS DISTRIBUTION CHECK
// ---------------------------------------------------------------------------
...
function weeklyDistributionCheck(employeeData, candidateShiftHours, settingsSnapshot, debugFlag) {
    ...
    // Get employee weekly hour limits
    ...
    // Calculate current weekly hours for this employee from globalAssignedCaregivers
    ...
    // FIXED: Calculate actual hours from existing assignments
    for (var assignmentKey in globalAssignedCaregivers) {
        if (globalAssignedCaregivers.hasOwnProperty(assignmentKey)) {
            ...
            if (normName(assignedCaregiver) === employeeNameNorm) {
                // Extract shift duration from assignment key: clientId_date_startTime
                ...
                if (keyParts.length >= 3) {
                    ...
                    // Find the actual shift duration from allClientSchedules
    ...
    // Max_Weekly_Hours is always enforced as hard stop regardless of mandatory flag
    ...
    // Calculate score adjustment based on target hours
    ...
    if (debugFlag && !result.reason) {
        result.reason = 'Weekly hours check passed. Current: ' + currentWeeklyHours +
            ', Candidate: ' + candidateShiftHours +
            ', Projected: ' + result.projectedHours +
...
// ---------------------------------------------------------------------------
// HELPER FUNCTIONS FOR CANDIDATE EVALUATION
// ---------------------------------------------------------------------------
...
function extractClientPrefs(clientData) {
    ...
    return {
        ...
        langs: parseList(safeGetValue(fields, 'Language_Preferences.value', '')), // Use consistent field name
        skills: parseList(safeGetValue(fields, 'Skills_Preferences.value', '')), // Use consistent field name  
        personality: parseList(safeGetValue(fields, 'Personality_Match.value', '')), // Check if this should be Personality_Preferences
...
// ---------------------------------------------------------------------------
// TIME PARSING AND CONFLICT DETECTION UTILITIES
// ---------------------------------------------------------------------------
...
function isCaregiverOnLeave(caregiverName, isoDate, leavesData, scheduleStartTime, scheduleEndTime) {
    ...
    if (!isArray(leavesData)) return false;
    ...
    for (var i = 0; i < leavesData.length; i++) {
        ...
        // Skip if different caregiver
        if (normName(leaveCaregiver) !== caregiverNameNorm) continue;

        // Skip if different date
        if (leaveDate !== isoDate) continue;

        // If we reach here, caregiver has leave on this date
        // Now check time overlap if both schedule and leave have time information

        if (scheduleStartTime !== undefined && scheduleEndTime !== undefined &&
            leaveStartTime && leaveEndTime) {

            // Parse leave times to minutes from midnight
            var leaveStartMin = parseTime(leaveStartTime);
            var leaveEndMin = parseTime(leaveEndTime);

            // If leave times are valid, check for time overlap
            if (leaveStartMin !== null && leaveEndMin !== null) {
                ...
                if (hasTimeOverlap) {
                    // Add debug information to track leave blocking
                    result.debug = result.debug || {};
                    ...
                    // No time overlap - caregiver is available during this schedule time
                    result.debug = result.debug || {};
                    result.debug.leaveBlockingInfo = result.debug.leaveBlockingInfo || [];
                    result.debug.leaveBlockingInfo.push({
                        caregiver: caregiverName,
        ...
        // If leave doesn't have time information or schedule doesn't have time info,
        // treat it as full day leave
        result.debug = result.debug || {};
        result.debug.leaveBlockingInfo = result.debug.leaveBlockingInfo || [];
        result.debug.leaveBlockingInfo.push({
            caregiver: caregiverName,
...
// ---------------------------------------------------------------------------
// MAIN ASSIGNMENT FUNCTION WITH PRIORITY-DRIVEN EVALUATION
// ---------------------------------------------------------------------------
...
function assignCaregiverToSchedule(clientData, dayObj, schedule, employeesDetails,
    ...
    // Calculate candidate shift hours
    var candidateShiftHours = (schedule.endTime - schedule.startTime) / 60;

    // Get priority settings for evaluation order
    var settings = getPrioritySettings(settingsTableData);
    var prefs = extractClientPrefs(clientData);

    // Get ALL caregiver names for evaluation
    var allNames = getAllCaregiverNames(employeesDetails);
    var availableCaregivers = [];
    var basicAvailableCaregivers = []; // Track caregivers who pass basic checks
    ...
    for (var ac = 0; ac < allNames.length; ac++) {
        ...
        // Step 1: Check basic availability (time slot availability)
        ...
        // Step 3: Check if assigned to other clients during this week (BEFORE conflict checking)
        var isAssignedToOtherClientThisWeek = false;
        for (var assignmentKey in globalAssignedCaregivers) {
            if (globalAssignedCaregivers.hasOwnProperty(assignmentKey)) {
                var assignedCaregiver = globalAssignedCaregivers[assignmentKey];
                var keyParts = assignmentKey.split('_');
                ...
                if (keyParts.length >= 3 &&
                    normName(assignedCaregiver) === normName(cand) &&
                    keyParts[0] !== clientId) { // Different client
                    isAssignedToOtherClientThisWeek = true;
                    break;
                }
            }
        }

        if (isAssignedToOtherClientThisWeek) {
            candidateResult.status = 'Not Available - Assigned to Other Client';
            candidateResult.rejectionReason = 'assigned_to_other_client_this_week';
            continue;
        }

        // Step 4: REAL-TIME conflict checking - Check against current assignments  
        ...
        if (candConflicts.length > 0) {
            ...
            candidateResult.conflictDetails = candConflicts; // Add conflict details for debugging
            continue;
        }

        // Step 5: Get employee record
        ...
        // Step 5: Check client blocklist (HARD CONSTRAINT)
        ...
        // Step 7: Check Max Weekly Hours (HARD CONSTRAINT)
        ...
        // PASSED ALL BASIC CHECKS - This caregiver can be assigned if needed
        candidateResult.passedBasicChecks = true;
        basicAvailableCaregivers.push(candidateResult);

        // Step 7: Priority-based evaluation
        ...
        // Calculate scores regardless of pass/fail
        ...
        if (passedPriorityCheck) {
            // Passed all priority checks - add to available list
            candidateResult.status = 'Available';
            availableCaregivers.push(candidateResult);
        } else {
            // Failed priority checks but still show in evaluation
    ...
    if (availableCaregivers.length === 0) {
        // No caregivers passed priority checks
        // Use ALL caregivers who passed basic checks as fallback options
        for (var i = 0; i < basicAvailableCaregivers.length; i++) {
            var candidate = basicAvailableCaregivers[i];

            // Mark as fallback and use only optional scores (no priority scores)
            candidate.status = 'Available (Fallback)';
            candidate.totalScore = candidate.optionalScore;
            candidate.priorityScore = 0; // Reset priority score for fallback

            // Add debug reason for fallback assignment
    ...
    // Sort available caregivers by their total score (best to worst)
    availableCaregivers.sort(function (a, b) {
        // Primary caregiver gets highest priority if available
        ...
        // Then by client hours (more experience with client)
        if (b.clientHours !== a.clientHours) return b.clientHours - a.clientHours;

        // Then by global hours (more experience overall)  
    ...
    // FIXED: Only show available caregivers (excluding the selected one) in availableCaregiversList
    ...
    for (var i = 0; i < availableCaregivers.length; i++) {
        ...
        if (i === 0) {
            // This is the selected caregiver - don't add to available list
            caregiver.rank = 1;
            caregiver.status = caregiver.status.indexOf('Fallback') !== -1 ? 'Selected (Fallback)' : 'Selected';
        } else {
            // FIXED: Check if this caregiver is already assigned to a different client this week
            var isAssignedToOtherClient = false;
            for (var assignmentKey in globalAssignedCaregivers) {
                if (globalAssignedCaregivers.hasOwnProperty(assignmentKey)) {
                    var assignedCaregiver = globalAssignedCaregivers[assignmentKey];
                    var keyParts = assignmentKey.split('_');
                    ...
                    if (keyParts.length >= 3 &&
                        normName(assignedCaregiver) === normName(caregiver.caregiverName) &&
                        keyParts[0] !== clientId) { // Different client
                        isAssignedToOtherClient = true;
                        break;
                    }
                }
            }

            // Only add to available list if not assigned to other clients
            if (!isAssignedToOtherClient) {
                caregiver.rank = availableCaregiversForList.length + 1;
                caregiver.status = caregiver.status.indexOf('Fallback') !== -1 ?
                    'Available (Fallback) - Rank ' + (availableCaregiversForList.length + 1) :
                    'Available - Rank ' + (availableCaregiversForList.length + 1);
                availableCaregiversForList.push(caregiver);
            } else {
                // Mark as unavailable due to other client assignment
                caregiver.rank = 0;
                caregiver.status = 'Unavailable - Assigned to Other Client';
                caregiver.rejectionReason = 'assigned_to_other_client_this_week';
            }
        }
    }

    // Set the result - show ONLY available caregivers (excluding selected)
    res.availableCaregiversList = availableCaregiversForList;

    if (availableCaregivers.length > 0) {
        // ASSIGN THE BEST AVAILABLE CAREGIVER (could be priority match or fallback)
        ...
        res.assignedCaregiverScore = bestCandidate.totalScore;

        // IMMEDIATELY store assignment to prevent conflicts in subsequent assignments
        ...
        // ENHANCEMENT: Track detailed time slot assignments for this caregiver
...
function checkHardConstraintsOnly(caregiverName, clientData, employeesDetails, candidateShiftHours) {
    var empRecord = getEmployeeRecordByName(caregiverName, employeesDetails);
    if (!empRecord || !empRecord.fields) {
        return { passes: false, reason: 'employee_not_found' };
    }

    // Check only Max Weekly Hours (always enforced)
    var weeklyResult = weeklyDistributionCheck(empRecord, candidateShiftHours, {}, false);
    if (weeklyResult.wouldExceedMax) {
        return { passes: false, reason: 'exceeds_max_weekly_hours' };
    }

    // Check client blocklist (always enforced)
    var blockResult = checkBlocklistStatus(caregiverName, clientData);
    if (!blockResult.passes) {
        return { passes: false, reason: 'blocked_by_client' };
    }

    return { passes: true, reason: 'hard_constraints_passed' };
}
...
function calculateTotalScore(debugReasons) {
    ...
    for (var i = 0; i < debugReasons.length; i++) {
        ...
        if (reason && typeof reason.scoreAdjustment === 'number') {
            ...
            // Categorize the score adjustments for better understanding
            var priority = reason.priority || '';
            if (priority.indexOf('Last Week') !== -1) {
                breakdown.lastWeek += reason.scoreAdjustment;
            } else if (priority.indexOf('Availability') !== -1) {
                breakdown.availability += reason.scoreAdjustment;
            } else if (priority.indexOf('Gender') !== -1) {
                breakdown.gender += reason.scoreAdjustment;
            } else if (priority.indexOf('Language') !== -1) {
                breakdown.language += reason.scoreAdjustment;
            } else if (priority.indexOf('Physical') !== -1) {
                breakdown.physical += reason.scoreAdjustment;
            } else if (priority.indexOf('Skills') !== -1) {
                breakdown.skills += reason.scoreAdjustment;
            } else if (priority.indexOf('Personality') !== -1) {
                breakdown.personality += reason.scoreAdjustment;
            } else if (priority.indexOf('Service Hours') !== -1) {
                breakdown.clientHours += reason.scoreAdjustment;
            } else if (priority.indexOf('Facility') !== -1) {
                breakdown.facility += reason.scoreAdjustment;
            } else if (priority.indexOf('Weekly') !== -1) {
                breakdown.weeklyHours += reason.scoreAdjustment;
            }
        }
    }
...
// Enhanced optional scoring with more detailed breakdown
function scoreOptional(prefs, profile) {
    if (!prefs || !profile) return 0;
    ...
    if (profile.langs.indexOf('english') !== -1) {
        details.english = 5;
        score += 5;
    }
    ...
    if (prefs.langs.length) {
        for (var i = 0; i < profile.langs.length; i++) {
            if (prefs.langs.indexOf(profile.langs[i]) !== -1) {
                details.languageMatch += 2;
                score += 2;
            }
        }
    }
    ...
    if (prefs.skills.length) {
        for (var j = 0; j < profile.skills.length; j++) {
            if (prefs.skills.indexOf(profile.skills[j]) !== -1) {
                details.skillsMatch += 1;
                score += 1;
            }
        }
    }
    ...
    if (prefs.personality.length) {
        for (var k = 0; k < profile.personality.length; k++) {
            if (prefs.personality.indexOf(profile.personality[k]) !== -1) {
                details.personalityMatch += 1;
                score += 1;
            }
        }
    }
...
// Priority-driven candidate evaluation function
function evaluateCandidateWithPriorities(candidateName, employeeData, candidateShiftHours, settings, clientData, debugReasons, isPrimary) {
    ...
    // Check if client is facility-based for priority handling
    var isFacility = isFacilityClient(clientData);

    // Get last week caregiver for this client
    ...
    // Iterate through priorities in the specified order
    for (var i = 0; i < settings.order.length; i++) {
        var priorityName = settings.order[i];

        // Only process active priorities
        ...
        // Handle Last Week Caregiver - SCORING ONLY, NOT BLOCKING
        if (priorityName === "Last Week Caregiver" || priorityName === "Last Week Caregivers") {
            debugEntry.allowed = true; // Always allowed, just affects scoring
            ...
            // Add score regardless of mandatory flag
            optionalScoreAdjustment += debugEntry.scoreAdjustment;
        }

        // Handle Caregiver Availability - SCORING ONLY
            ...
            debugEntry.allowed = true; // Always allowed for priority scoring
        ...
        // Handle Gender Preference - ENFORCE ONLY IF CLIENT HAS STRICT REQUIREMENT
            ...
            // Only block if client has strict gender requirement OR it's marked mandatory
        ...
        // Handle Language Preference - SCORING ONLY UNLESS MANDATORY
            ...
            // Only block if marked as mandatory AND client has language requirements
            var clientFields = clientData.fields || {};
        ...
        // Handle Physical Capability - ENFORCE ONLY IF CLIENT HAS REQUIREMENT
            ...
            // Only block if client has physical requirements
            var clientFields = clientData.fields || {};
        ...
        // Handle Skills - SCORING ONLY UNLESS MANDATORY
            ...
            // Only block if marked as mandatory AND client has skill requirements
            var clientFields = clientData.fields || {};
        ...
        // Handle Personality Match - SCORING ONLY
        ...
        // Handle Blocklisted Caregivers - ALWAYS ENFORCED
            ...
            debugEntry.scoreAdjustment = 0; // Blocklist doesn't affect scoring, just eligibility
            debugEntry.reason = blockResult.reason || 'blocklist_checked';

            // Always enforce blocklist regardless of mandatory flag
            if (!blockResult.passes) {
                allowed = false;
                debugReasons.push(debugEntry);
                return false;
        ...
        // Handle Max Service Hours with Client - SCORING ONLY
            ...
            var scoreBoost = Math.min(clientHours * 0.5, 15); // Cap at 15 points
        ...
        // Handle Facility Clients Priority - SCORING ONLY
        ...
        // Handle Weekly Min/Max Hours Compliance - ENFORCE MAX HOURS ONLY
            ...
            // Max_Weekly_Hours is always enforced as hard stop
            if (weeklyResult.wouldExceedMax) {
                allowed = false;
                debugReasons.push(debugEntry);
                return false;
        ...
        // Handle unknown priorities - SCORING ONLY
        else {
...
// Check caregiver availability (basic implementation)
...
function checkBlocklistStatus(caregiverName, clientData) {
    ...
    if (blockList.length === 0) {
        return { passes: true, reason: 'no_blocklist' };
    }
...
// Fix field name mismatches in helper functions
function extractClientPrefs(clientData) {
    if (!clientData || !clientData.fields) return {};
    var fields = clientData.fields;
    return {
        ...
        genderPrefNorm: normStr(safeGetValue(fields, 'Gender_Preference.value', '')).toLowerCase(),
        physReq: safeParseNumber(safeGetValue(fields, 'Physical_Capability_lbs.value', 0), 0),
        langs: parseList(safeGetValue(fields, 'Language_Preferences.value', '')), // Fixed field name
        skills: parseList(safeGetValue(fields, 'Skills_Preferences.value', '')), // Fixed field name
        personality: parseList(safeGetValue(fields, 'Personality_Match.value', '')),
        blockList: parseList(safeGetValue(fields, 'Caregiver_Block_List.value', ''))
    };
}

// Enhanced Language Preference Check - Fixed version
function checkLanguagePreference(caregiverName, clientData, employeesDetails, isMandatory) {
    var clientFields = clientData.fields || {};
    var requiredLangs = parseList(safeGetValue(clientFields, 'Language_Preferences.value', '')); // Fixed field name
    ...
    for (var i = 0; i < requiredLangs.length; i++) {
        if (caregiverLangs.indexOf(requiredLangs[i]) !== -1) {
            matchCount++;
        }
    }
...
// Enhanced Skills Check - Fixed version
function checkSkillsRequirement(caregiverName, clientData, employeesDetails, isMandatory) {
    var clientFields = clientData.fields || {};
    var requiredSkills = parseList(safeGetValue(clientFields, 'Skills_Preferences.value', '')); // Fixed field name
    ...
    for (var i = 0; i < requiredSkills.length; i++) {
        if (caregiverSkills.indexOf(requiredSkills[i]) !== -1) {
            matchCount++;
        }
    }
...
// Enhanced Personality Check - Fixed version  
function checkPersonalityMatch(caregiverName, clientData, employeesDetails, isMandatory) {
    ...
    for (var i = 0; i < requiredPersonality.length; i++) {
        if (caregiverPersonality.indexOf(requiredPersonality[i]) !== -1) {
            matchCount++;
        }
    }
...
// Enhanced Gender Preference Check - Fixed version
function checkGenderPreference(caregiverName, clientData, employeesDetails, isMandatory) {
    ...
    // Enforce if mandatory OR if client has strict preference
...
// Simple conflict checking function (placeholder - implement based on your conflict detection logic)
function checkScheduleConflicts(caregiverName, targetClientId, targetStartTime, targetEndTime, targetDate, allSchedules, globalAssignedCaregivers) {
    ...
    // Check if caregiver is already assigned to another client at the SAME time
    if (globalCaregiverTimeSlots[caregiverName]) {
        ...
        for (var i = 0; i < assignments.length; i++) {
            ...
            // Skip if it's the same client - allow same client to have multiple shifts
            if (assignment.clientId === targetClientId) continue;

            // Check if date is the same and there's a time overlap
    ...
    // Existing conflict check logic as a backup
    for (var key in globalAssignedCaregivers) {
        if (globalAssignedCaregivers.hasOwnProperty(key) && globalAssignedCaregivers[key] === caregiverName) {
            var keyParts = key.split('_');
            if (keyParts.length >= 3) {
                var existingClientId = keyParts[0];
                var existingDate = keyParts[1];
                var existingStartTime = parseInt(keyParts[2], 10);

                // Skip if it's the same client (allow same client to have multiple shifts)
                if (existingClientId === targetClientId) continue;

                // FIXED: Check for ANY existing assignment to a different client (not just same day)
                // This prevents showing caregivers as "available" when they're already assigned to other clients
                if (existingDate === targetDate) {
                    // Same day - check for time overlap
                    var existingEndTime = existingStartTime + 120; // Default 2 hours if not found

                    if (allSchedules[targetDate] && allSchedules[targetDate][existingClientId]) {
                        var clientSchedules = allSchedules[targetDate][existingClientId];
                        for (var i = 0; i < clientSchedules.length; i++) {
                            var schedule = clientSchedules[i];
                            if (schedule.startTime === existingStartTime) {
                                existingEndTime = schedule.endTime;
                                break;
                            }
                        }
                    }
                    ...
                    if (timeOverlap(targetStartTime, targetEndTime, existingStartTime, existingEndTime)) {
                        conflicts.push({
                            ...
                            conflictType: 'time_overlap',
                            date: targetDate,
                            existingClient: existingClientId,
                            existingTime: existingStartTime + '-' + existingEndTime,
                            targetTime: targetStartTime + '-' + targetEndTime
                        });
                    }
                } else {
                    // Different day - mark as unavailable if already assigned to a different client
                    // This is the key fix: prevent showing as "available" for other clients
                    conflicts.push({
                        caregiverName: caregiverName,
                        conflictType: 'weekly_assignment_conflict',
                        date: targetDate,
                        existingClient: existingClientId,
                        existingDate: existingDate,
                        existingTime: existingStartTime,
                        targetTime: targetStartTime + '-' + targetEndTime,
                        reason: 'caregiver_already_assigned_to_different_client_this_week'
                    });
                }
            }
        }
...
// Add a new function for second-pass processing of availableCaregiversList
    ...
    for (var c = 0; c < clientAssignments.length; c++) {
        ...
        for (var s = 0; s < clientAssignment.scheduledServices.length; s++) {
            ...
            for (var i = 0; i < availableList.length; i++) {
                ...
                // Check if this caregiver has conflicts at this specific date/time
                if (globalCaregiverTimeSlots[caregiver.caregiverName]) {
                    var assignments = globalCaregiverTimeSlots[caregiver.caregiverName];
                    ...
                    for (var j = 0; j < assignments.length; j++) {
                        ...
                        // Check if there's a time overlap for this specific date
                        if (assignment.date === service.date &&
                            timeOverlap(service.startTime, service.endTime, assignment.startTime, assignment.endTime)) {
                            hasConflict = true;
                            break;
                        }
                    }
            ...
            // Update the service with the filtered list
...
// ---------------------------------------------------------------------------
// NEW PRIORITY IMPLEMENTATION FUNCTIONS
// ---------------------------------------------------------------------------

// 1. Last Week Caregiver Detection
function getLastWeekCaregiver(clientData, actualSchedulingData) {
    ...
    // Manual date calculation to avoid js2py Date parsing issues
    ...
    // Calculate last week date range (7-14 days ago)
    ...
    for (var i = 0; i < actualSchedulingData.length; i++) {
        ...
        if (status === 'Completed' &&
            ...
            caregiverName &&
            scheduleDate >= lastWeekStartISO &&
            scheduleDate <= lastWeekEndISO) {
...
// 2. Calculate Max Service Hours with specific client
...
// 3. Check if client is facility-based
...
// 4. Enhanced Gender Preference Check
function checkGenderPreference(caregiverName, clientData, employeesDetails, isMandatory) {
    var clientFields = clientData.fields || {};
    var genderPref = safeGetValue(clientFields, 'Gender_Preference.value', '');
    var isStrict = normStr(safeGetValue(clientFields, 'Gender_Preference_Strict.value', '')).toLowerCase() === 'yes';

    if (!genderPref) return { passes: true, reason: 'no_gender_preference' };
...
// 5. Enhanced Language Preference Check
function checkLanguagePreference(caregiverName, clientData, employeesDetails, isMandatory) {
    var clientFields = clientData.fields || {};
    var requiredLangs = parseList(safeGetValue(clientFields, 'Language_Preference.value', ''));

    if (requiredLangs.length === 0) return { passes: true, reason: 'no_language_preference', scoreBoost: 0 };
...
// 6. Enhanced Physical Capability Check (Weight Class)
...
// 7. Enhanced Skills Check
function checkSkillsRequirement(caregiverName, clientData, employeesDetails, isMandatory) {
    var clientFields = clientData.fields || {};
    var requiredSkills = parseList(safeGetValue(clientFields, 'Skills_Preference.value', ''));

    if (requiredSkills.length === 0) return { passes: true, reason: 'no_skills_requirement', scoreBoost: 0 };
...
// 8. Enhanced Personality Check
function checkPersonalityMatch(caregiverName, clientData, employeesDetails, isMandatory) {
    var clientFields = clientData.fields || {};
    var requiredPersonality = parseList(safeGetValue(clientFields, 'Personality_Preference.value', ''));

    if (requiredPersonality.length === 0) return { passes: true, reason: 'no_personality_preference', scoreBoost: 0 };
...
function buildAllClientSchedules(allClientsData, clientSchedulesData, next7Days) {
    ...
    for (var i = 0; i < schedulesList.length; i++) {
        ...
        // Add schedule for matching days
...
// Sort clients - Private first, then Facility
    ...
    for (var dayIndex = 0; dayIndex < next7Days.length; dayIndex++) {
        ...
        for (var scheduleIndex = 0; scheduleIndex < clientScheduleRows.length; scheduleIndex++) {
            ...
            if (schedule.day === dayObj.day) {
                ...
                if (isAvailable) {
                    serviceObj.caregiverScheduledHours = Math.round(requestedHours * 100) / 100;
                }
...
// ENHANCEMENT: Second pass to fix availableCaregiversList
...
// Generate caregiver utilization report
