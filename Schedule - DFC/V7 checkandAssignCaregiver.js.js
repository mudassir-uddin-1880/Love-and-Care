// Full working code but withoiut ghost shifting scheduling
...
function isCaregiverAvailable(caregiverName, targetClientId, targetDate, targetStartTime, targetEndTime) {
    // Early return if no caregiver name or date provided
    if (!caregiverName || !targetDate) return true;

    // Check if caregiver has any time slot assignments
...
// Configure historical lookback days
var HISTORICAL_LOOKBACK_DAYS = safeParseNumber(
    safeGetValue((settingsrecords[0] && settingsrecords[0].fields) || {}, 'Historical_Lookback_Days.value', 30),
    30
);

// Utility: count historical shifts (Approved/Completed) in past N days for caregiver+client
function getHistoricalCount(caregiverName, clientName, actualSchedulingData, lookbackDays) {
    ...
    var now = currDate;
    if (!now) return 0;
    var cutoff; // YYYY-MM-DD simple lexicographic compare ok
    (function calcCutoff() {
        // crude subtract lookbackDays (assumes same month boundaries rarely critical for relative ordering)
        var p = now.split('-'); // [yyyy, mm, dd]
...
function scoreOptional(prefs, profile) {
    ...
    if (prefs.personality.length) {
        for (var k = 0; i < profile.personality.length; k++) {
...
function assignCaregiverToSchedule(clientData, dayObj, schedule, employeesDetails,
    ...
    var dimensionStats = []; // collect per-candidate raw dimension metrics
    ...
    for (var ac = 0; ac < allNames.length; ac++) {
        ...
        // IMPROVED STEP 3: Use centralized availability check instead of weekly assignment check
        // Check if available for this specific date/time slot
        ...
        // Step 4: Get detailed conflicts (mainly for debugging/reporting)
        ...
        // Step 6: Check client blocklist (HARD CONSTRAINT)
        ...
        // Step 8: Priority-based evaluation
        ...
        // Compute raw metrics directly for weighted scoring
    ...
    // Apply weighted scoring before sorting
    if (availableCaregivers.length > 0) {
        var rawMap = {};
        ...
        // Restrict dimension maxima to only those actually in availableCaregivers
        ...
        // Sort available caregivers by weighted score, then by primary preference
        availableCaregivers.sort(function (a, b) {
            // First sort by weighted total score (highest first)
            if (b.weightedTotalScore !== a.weightedTotalScore) return b.weightedTotalScore - a.weightedTotalScore;

            // Primary caregiver gets preference only when scores tie
            if (a.isPrimary !== b.isPrimary) return a.isPrimary ? -1 : 1;

            // Then by total score as fallback
    ...
    for (var i = 0; i < availableCaregivers.length; i++) {
        ...
        if (i === 0) {
            ...
            // Check if this caregiver is already assigned to a different client this week
            ...
            if (isAvailableForAssignment) {
                caregiver.rank = availableCaregiversForList.length + 2; // Start from 2 since selected is rank 1
                ...
                // Mark as unavailable due to time conflict
                caregiver.rank = 0;
    ...
    if (availableCaregivers.length > 0) {
        ...
        // Expose weighted info to service layer
        ...
        // Update globalCaregiverHours after successful assignment
        var shiftHours = (schedule.endTime - schedule.startTime) / 60;
        globalCaregiverHours[bestCandidate.caregiverName] =
            (globalCaregiverHours[bestCandidate.caregiverName] || 0) + shiftHours;

        // Track client-specific hours if desired
...
// Simple conflict checking function using centralized availability check
function checkScheduleConflicts(caregiverName, targetClientId, targetStartTime, targetEndTime, targetDate, allSchedules, globalAssignedCaregivers) {
    ...
    if (!isCaregiverAvailable(caregiverName, targetClientId, targetDate, targetStartTime, targetEndTime)) {
        // If not available, find the specific conflict(s) for reporting
        if (globalCaregiverTimeSlots[caregiverName]) {
            var assignments = globalCaregiverTimeSlots[caregiverName];
    ...
    for (var c = 0; c < clientAssignments.length; c++) {
        ...
        for (var s = 0; s < clientAssignment.scheduledServices.length; s++) {
            ...
            for (var i = 0; i < availableList.length; i++) {
                ...
                // Use centralized availability check instead of manual checking
...
// Process each client
var globalCaregiverHours = {}; // Maintain shared globalCaregiverHours
        ...
        for (var scheduleIndex = 0; scheduleIndex < clientScheduleRows.length; scheduleIndex++) {
            ...
            if (schedule.day === dayObj.day) {
                var assignmentResult = assignCaregiverToSchedule(
                    ...
                    {}, // clientCaregiverHours (can keep per-client if you want a map)
                    globalCaregiverHours, // Pass shared globalCaregiverHours
                ...
                var serviceObj = {
                    ...
                    // Add weighted data to service object output
