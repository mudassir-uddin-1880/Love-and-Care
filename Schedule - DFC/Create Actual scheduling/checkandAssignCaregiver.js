// Multiple Caregivers Scheduling Validation Script

var settingsrecords = input.settingsrecords || [];
var settingsTableData = input.settingsTableData || [];
var currDate = "2025-12-26"; //input.currDate;
...
function parseTime(timeStr) {
    ...
    // Replace semicolons with colons to handle common data entry errors
    var cleanTime = normStr(timeStr).replace(/;/g, ':').replace(/[^0-9:]/g, '');
...
function isClientOnLeave(clientId, scheduleDate, scheduleStartTime, scheduleEndTime, clientLeaves) {
    ...
    // Use pre-computed map - CRITICAL: clientId must match exactly
    var clientApprovedLeaves = clientLeaveMap[clientId];

    // Debug: Log when checking leaves
    if (!result.debug.clientLeaveChecks) {
        result.debug.clientLeaveChecks = [];
    }

    if (!clientApprovedLeaves || clientApprovedLeaves.length === 0) {
        result.debug.clientLeaveChecks.push({
            clientId: clientId,
            scheduleDate: scheduleDate,
            scheduleTime: minutesToHHMM(scheduleStartTime) + '-' + minutesToHHMM(scheduleEndTime),
            result: 'no_leaves_found',
            availableInMap: Object.keys(clientLeaveMap)
        });
        return false;
    }
    ...
    for (var i = 0; i < clientApprovedLeaves.length; i++) {
        ...
        // If both schedule and leave have specific times, check for time overlap
            ...
            if (leaveStartMin !== null && leaveEndMin !== null) {
                if (timeOverlap(scheduleStartTime, scheduleEndTime, leaveStartMin, leaveEndMin)) {
                    result.debug.clientLeaveChecks.push({
                        ...
                        scheduleDate: scheduleDate,
                        scheduleTime: minutesToHHMM(scheduleStartTime) + '-' + minutesToHHMM(scheduleEndTime),
                        leaveTime: leaveStartTime + '-' + leaveEndTime,
                        result: 'blocked_by_leave',
                        overlap: true
                    });
                    return true;
                }
            } else {
                // Leave times are invalid - treat as full-day leave
                result.debug.clientLeaveChecks.push({
                    clientId: clientId,
                    scheduleDate: scheduleDate,
                    scheduleTime: minutesToHHMM(scheduleStartTime) + '-' + minutesToHHMM(scheduleEndTime),
                    result: 'blocked_by_leave',
                    reason: 'invalid_leave_times_treated_as_full_day'
                });
                return true;
            }
        } else {
            // Either schedule or leave doesn't have times - treat as full-day leave
            result.debug.clientLeaveChecks.push({
                clientId: clientId,
                scheduleDate: scheduleDate,
                scheduleTime: scheduleStartTime !== undefined ?
                    minutesToHHMM(scheduleStartTime) + '-' + minutesToHHMM(scheduleEndTime) : 'no_time',
                result: 'blocked_by_leave',
                reason: 'full_day_leave_or_missing_schedule_time'
            });
            return true;
        }
    }
    ...
    result.debug.clientLeaveChecks.push({
        ...
        scheduleTime: minutesToHHMM(scheduleStartTime) + '-' + minutesToHHMM(scheduleEndTime),
        result: 'allowed',
        reason: 'no_overlapping_leaves'
    });

    return false;
}

function isCaregiverActive(caregiverName, employeesDetails) {
    ...
    if (!empRecord || !empRecord.fields) return false;

    // Check common active status field names
    var status = normStr(safeGetValue(empRecord.fields, 'Status.value', '')).toLowerCase();
    var isActive = normStr(safeGetValue(empRecord.fields, 'Active.value', '')).toLowerCase();
    var employeeStatus = normStr(safeGetValue(empRecord.fields, 'Employee_Status.value', '')).toLowerCase();

    // If any status field explicitly says "inactive" or "terminated", reject
    if (status === 'inactive' || status === 'terminated' ||
        isActive === 'no' || isActive === 'false' ||
        employeeStatus === 'inactive' || employeeStatus === 'terminated') {
        return false;
    }

    // Default to active if no status field exists (backward compatibility)
    return true;
}
...
function isCaregiverAvailable(caregiverName, targetClientId, targetDate, targetStartTime, targetEndTime) {
    ...
    // CRITICAL: Normalize caregiver name for lookup consistency
    var normalizedCaregiver = normName(caregiverName);

    if (globalCaregiverTimeSlots[normalizedCaregiver]) {
        var assignments = globalCaregiverTimeSlots[normalizedCaregiver];  // ✅ Keep only this line

        for (var i = 0; i < assignments.length; i++) {
            var assignment = assignments[i];

            // NEW: Allow same client at same time (multi-caregiver schedules)
            if (assignment.clientId === targetClientId &&
                assignment.date === targetDate &&
                timeOverlap(targetStartTime, targetEndTime, assignment.startTime, assignment.endTime)) {
                continue; // Same client, same time - allowed for multi-caregiver schedules
            }
...
function getLastWeekCaregiver(clientData, actualSchedulingData) {
    ...
    for (var i = 0; i < actualSchedulingData.length; i++) {
        ...
        var caregiverName = safeGetValue(fields, 'Expected_Caregiver.value', '');
        ...
        if ((status === 'Completed' || status === 'Scheduled Completed') &&
            ...
            caregiverNameNorm && scheduleDate >= lastWeekStartISO && scheduleDate <= lastWeekEndISO) {

            // Initialize caregiver tracking
            if (!caregiverHours[caregiverNameNorm]) {
                caregiverHours[caregiverNameNorm] = 0;
                caregiverLastDate[caregiverNameNorm] = scheduleDate;
            }


            // Add hours to caregiver's total
            caregiverHours[caregiverNameNorm] += expectedHours;

            // Keep track of most recent date for this caregiver
            if (scheduleDate > caregiverLastDate[caregiverNameNorm]) {
                caregiverLastDate[caregiverNameNorm] = scheduleDate;
            }
    ...
    if (maxHoursCaregiver) {
        for (var i = 0; i < actualSchedulingData.length; i++) {
            var record = actualSchedulingData[i];
            if (!record || !record.fields) continue;
            var caregiverName = safeGetValue(record.fields, 'Expected_Caregiver.value', '');
            if (normName(caregiverName) === maxHoursCaregiver) {
                return caregiverName; // Return original formatted name
            }
        }
    }
...
function getLastWeekCompleteSchedule(clientData, actualSchedulingData, targetWeekDays) {
    ...
    for (var j = 0; j < actualSchedulingData.length; j++) {
        ...
        var caregiverName = safeGetValue(fields, 'Expected_Caregiver.value', '');
        var scheduleDate = safeGetValue(fields, 'Schedule_Start_Date.value', '');
        var recordClientNameNorm = normName(recordClientName);
        var caregiverNameNorm = normName(caregiverName);
        ...
        if ((status === 'Completed' || status === 'Scheduled Completed' || status === 'Approved') &&
            recordClientNameNorm === clientNameNorm &&
            caregiverNameNorm && scheduleDate && startTime && endTime) {
...
function assignGhostCaregiversToShifts(shifts, employeesDetails, caregiverTimeSlots) {
    ...
    // Track ghost shift assignments per caregiver per day
    var caregiverDayAssignments = {};
    var caregiverWeekAssignments = {};
    eligibleCaregivers.forEach(function (cg) {
        var normalizedName = normName(cg.name);
        caregiverDayAssignments[normalizedName] = {};
        caregiverWeekAssignments[normalizedName] = 0;
    });
    ...
    for (var i = 0; i < shifts.length; i++) {
        ...
        while (!assigned && attempts < totalCaregivers) {
            ...
            var normalizedCaregiverName = normName(caregiver.name);
            ...
            if (caregiverDayAssignments[normalizedCaregiverName][shift.date]) {
                caregiverIndex++;
                attempts++;
                continue;
            }
            ...
            // ✅ NEW LOGIC: Check if caregiver has TIME OVERLAP with client assignments
            // Instead of blocking the entire day, only block if there's a time conflict
            var hasTimeConflict = false;
            if (caregiverTimeSlots[normalizedCaregiverName]) {
                var assignments = caregiverTimeSlots[normalizedCaregiverName];
                for (var a = 0; a < assignments.length; a++) {
                    var assignment = assignments[a];
                    if (assignment.date === shift.date) {
                        // Check if ghost shift time overlaps with client assignment time
                        if (timeOverlap(shift.startTime, shift.endTime, assignment.startTime, assignment.endTime)) {
            ...
            caregiverDayAssignments[normalizedCaregiverName][shift.date] = true;
            caregiverWeekAssignments[normalizedCaregiverName]++;
            assigned = true;
            caregiverIndex++;
        }
    }
...
function assignCaregiverToSchedule(clientData, dayObj, schedule, employeesDetails,
    ...
    clientCaregiverHours, globalCaregiverHours, actualSchedulingData, isSubsequentSlot,
    alreadyAssignedCaregivers) {
    ...
    var slotIndex = (typeof isSubsequentSlot === 'number') ? isSubsequentSlot : 0;
    ...
    if (lastWeekCompleteSchedule && lastWeekCompleteSchedule[dayObj.day]) {
        ...
        // Find ALL matching time slots from last week
        ...
        // Try to use the caregiver for this specific slot
        var matchingAssignment = matchingAssignments[slotIndex] || null;


        if (matchingAssignment) {
            var lastWeekCaregiver = matchingAssignment.caregiverName;
            var normalizedLastWeekCaregiver = normName(lastWeekCaregiver);

            // **NEW: Check if this caregiver is already assigned to another slot in this schedule**
            var isDuplicate = false;
            if (alreadyAssignedCaregivers && isArray(alreadyAssignedCaregivers)) {
                for (var i = 0; i < alreadyAssignedCaregivers.length; i++) {
                    if (normName(alreadyAssignedCaregivers[i]) === normalizedLastWeekCaregiver) {
                        isDuplicate = true;
                        break;
                    }
                }
            ...
            if (isDuplicate) {
                res.debugReasons.push({
                    priority: 'Last Week Schedule Copy Skipped',
                    allowed: false,
                    scoreAdjustment: 0,
                    reason: 'Caregiver ' + lastWeekCaregiver + ' already assigned to another slot in this schedule. Skipping to avoid duplicate assignment.'
                });
                // Don't return here - let it fall through to normal assignment
            } else {
                res.debugReasons.push({
                    priority: 'Last Week Schedule Copy',
                    allowed: true,
                    scoreAdjustment: 0,
                    reason: 'Found exact matching schedule from last week (slot ' + (slotIndex + 1) + ' of ' + matchingAssignments.length + '): ' + lastWeekCaregiver +
            ...
            } // End of else block for !isDuplicate
            ...
            // 2a. **FIX: Check if caregiver is still active**
            if (canCopyAssignment) {
                if (!isCaregiverActive(lastWeekCaregiver, employeesDetails)) {
                    canCopyAssignment = false;
                    conflictReason = 'caregiver_inactive_in_employee_list';
                }
            }
            ...
            // REMOVED CHECKS (Skip for last week caregiver):
            // - Weekly hours check (weeklyDistributionCheck)
            // - Time slot availability (isCaregiverAvailableForSchedule)
            // - Client type compatibility
            // - Physical capability (mandatory)
            // - Language preference (mandatory)
            // - Skills requirements (mandatory)

            if (canCopyAssignment) {
                // SUCCESS! Copy the exact assignment from last week
                var normalizedLastWeekCaregiver = normName(lastWeekCaregiver);  // ✅ MOVE THIS UP
                ...
                res.debugReasons.push({
                    ...
                    reason: 'Successfully copied exact schedule from last week: ' + lastWeekCaregiver +
                        ' (passed critical validation checks: leave status, blocklist, time conflicts. Skipped: weekly hours, mandatory requirements)'
                });

                var assignmentKey = clientId + '_' + dayObj.iso + '_' + schedule.startTime + '_slot' + caregiverSlot;  // ✅ ADD SLOT INDEX
                globalAssignedCaregivers[assignmentKey] = normalizedLastWeekCaregiver;  // ✅ USE NORMALIZED

                var shiftHours = Math.round(((schedule.endTime - schedule.startTime) / 60) * 10) / 10;
                globalCaregiverHours[normalizedLastWeekCaregiver] =  // ✅ USE NORMALIZED
                    (globalCaregiverHours[normalizedLastWeekCaregiver] || 0) + shiftHours;

                clientCaregiverHours[normalizedLastWeekCaregiver] =  // ✅ USE NORMALIZED
                    (clientCaregiverHours[normalizedLastWeekCaregiver] || 0) + shiftHours;

                if (!globalCaregiverTimeSlots[normalizedLastWeekCaregiver]) {  // ✅ USE NORMALIZED
                    globalCaregiverTimeSlots[normalizedLastWeekCaregiver] = [];
                }

                globalCaregiverTimeSlots[normalizedLastWeekCaregiver].push({  // ✅ USE NORMALIZED
                    clientId: clientId,
                    date: dayObj.iso,
                ...
                // Calculate proper weighted score for last week caregiver
                var clientHours = calculateClientSpecificHours(lastWeekCaregiver, clientData, actualSchedulingData);
                ...
                var workedHours = actualWorkedHoursMap[normStr(lastWeekCaregiver)] || 0;
                var workHoursScore = 0;
                if (totalClientHours > 0) {
                    workHoursScore = (workedHours / totalClientHours) * scoringWeights.workHours;
                }

                var prefs = extractClientPrefs(clientData);
                var empRecord = getEmployeeRecordByName(lastWeekCaregiver, employeesDetails);
                var profile = getCaregiverProfile(empRecord);

                var totalClientLangs = prefs.langs.length;
                var totalClientSkills = prefs.skills.length;

                // FIX: Count actual language matches
                var rawLanguageMatches = 0;
                if (totalClientLangs > 0 && profile && profile.langs.length) {
                    for (var li = 0; li < prefs.langs.length; li++) {
                        if (profile.langs.indexOf(prefs.langs[li]) !== -1) rawLanguageMatches++;
                    }
                }

                // FIX: Count actual skill matches
                var rawSkillMatches = 0;
                if (totalClientSkills > 0 && profile && profile.skills.length) {
                    for (var si = 0; si < prefs.skills.length; si++) {
                        if (profile.skills.indexOf(prefs.skills[si]) !== -1) rawSkillMatches++;
                    }
                }

                var rawHistoricalCount = getHistoricalCount(lastWeekCaregiver, clientName, actualSchedulingData, HISTORICAL_LOOKBACK_DAYS);

                // FIX: Use PROPORTIONAL calculation (not full points)
                var langScore = (totalClientLangs > 0)
                    ? (rawLanguageMatches / totalClientLangs) * scoringWeights.language
                    : 0;
                var skillScore = (totalClientSkills > 0)
                    ? (rawSkillMatches / totalClientSkills) * scoringWeights.skills
                    : 0;
                var histScore = (rawHistoricalCount > 0)
                    ? scoringWeights.historical  // Last week caregiver gets full historical points
                    : 0;

                var weightedBreakdown = {
                ...
                var weightedTotal = +(langScore + skillScore + histScore + workHoursScore).toFixed(2);

                res.assignedCaregiverScore = weightedTotal;
                res.assignedCaregiverWeightedTotal = weightedTotal;
                res.assignedCaregiverWeightedBreakdown = weightedBreakdown;
            ...
            res.debugReasons.push({
                ...
                allowed: false,
                scoreAdjustment: 0,
                reason: 'No matching caregiver for slot ' + (slotIndex + 1) + ' in last week schedule for ' + dayObj.day +
                    ' ' + minutesToHHMM(schedule.startTime) + '-' + minutesToHHMM(schedule.endTime) +
                    ' (found ' + matchingAssignments.length + ' last week caregiver(s), but this slot index exceeds available assignments)' +
                    '. Proceeding with normal assignment process.'
            });
        }
    } else {
    ...
    var shouldTryIndividualLastWeek = (slotIndex > 0 || !matchingAssignment);
    var lastWeekCaregiver = shouldTryIndividualLastWeek ? getLastWeekCaregiver(clientData, actualSchedulingData) : null;

    if (lastWeekCaregiver) {
        var normalizedLastWeekCaregiver = normName(lastWeekCaregiver);
        // **FIX: Check if last week caregiver is still active**
        if (!isCaregiverActive(lastWeekCaregiver, employeesDetails)) {
            res.debugReasons.push({
                priority: 'Last Week Caregiver Inactive',
                allowed: false,
                scoreAdjustment: 0,
                reason: 'Last week caregiver (' + lastWeekCaregiver + ') is no longer active in employee list. Proceeding with normal assignment.'
            });
            lastWeekCaregiver = null; // Clear it so we proceed to normal assignment
        }

        // **NEW: Check if this caregiver is already assigned to another slot**
        var isDuplicate = false;
        if (alreadyAssignedCaregivers && isArray(alreadyAssignedCaregivers)) {
            for (var i = 0; i < alreadyAssignedCaregivers.length; i++) {
                if (normName(alreadyAssignedCaregivers[i]) === normalizedLastWeekCaregiver) {
        ...
        if (isDuplicate) {
            res.debugReasons.push({
                priority: 'Last Week Caregiver Already Assigned',
                allowed: false,
                scoreAdjustment: 0,
                reason: 'Last week caregiver ' + lastWeekCaregiver + ' already assigned to another slot. Proceeding with normal assignment.'
            });
            lastWeekCaregiver = null; // Clear it so we proceed to STEP 3
        } else {
            res.debugReasons.push({
                priority: 'Last Week Caregiver Found',
                allowed: true,
                scoreAdjustment: 0,
                reason: 'Found last week caregiver: ' + lastWeekCaregiver + '. Checking critical conditions only...'
            });
        }
    }
    ...
    if (lastWeekCaregiver) {
        ...
        if (canAssignLastWeekCaregiver) {
            ...
            // Update global tracking - use normalized name consistently
            var assignmentKey = clientId + '_' + dayObj.iso + '_' + schedule.startTime + '_slot' + caregiverSlot;  // ✅ ADD SLOT INDEX
            globalAssignedCaregivers[assignmentKey] = normalizedLastWeekCaregiver;  // ✅ USE NORMALIZED
    ...
    for (var ac = 0; ac < allNames.length; ac++) {
        ...
        // **FIX: Check if caregiver is active**
        if (!isCaregiverActive(cand, employeesDetails)) {
            candidateResult.status = 'Not Available - Inactive';
            candidateResult.rejectionReason = 'caregiver_inactive';
            continue;
        }
        ...
        var normalizedCandName = normName(cand);

        dimensionStats.push({
            caregiverName: cand,
            caregiverNameNormalized: normalizedCandName,  // NEW
    ...
    if (availableCaregivers.length > 0) {
        ...
        // --- Calculate weighted language and skill scores ---
        availableCaregivers.forEach(function (c) {
            var metrics = rawMap[c.caregiverName];

            // CRITICAL FIX: If exact match fails, try normalized match
            if (!metrics || typeof metrics.rawLanguage === 'undefined') {
                var normalizedCgName = normName(c.caregiverName);
                for (var key in rawMap) {
                    if (normName(key) === normalizedCgName) {
                        metrics = rawMap[key];
                        break;
                    }
                }
            ...
            // If still not found, use empty metrics and log error
            if (!metrics || typeof metrics.rawLanguage === 'undefined') {
                result.debug = result.debug || {};
                result.debug.lookupErrors = result.debug.lookupErrors || [];
                result.debug.lookupErrors.push({
                    caregiver: c.caregiverName,
                    normalized: normName(c.caregiverName),
                    mapKeys: Object.keys(rawMap),
                    dimensionStatsSize: dimensionStats.length
                });
                metrics = { rawLanguage: 0, rawSkills: 0, rawHistorical: 0, rawWorkHours: 0 };
            }
            ...
            var rawLangVal = (typeof metrics.rawLanguage === 'number') ? metrics.rawLanguage : 0;
            var rawSkillVal = (typeof metrics.rawSkills === 'number') ? metrics.rawSkills : 0;
            var rawHistVal = (typeof metrics.rawHistorical === 'number') ? metrics.rawHistorical : 0;

            // Calculate proportional weighted scores
            var langScore = 0;
            if (totalClientLangs > 0) {
                langScore = (rawLangVal / totalClientLangs) * scoringWeights.language;
            }

            var skillScore = 0;
            if (totalClientSkills > 0) {
                skillScore = (rawSkillVal / totalClientSkills) * scoringWeights.skills;
            }

            var histScore = rawHistVal > 0 ? scoringWeights.historical : 0;

            var wWork = c.weightedBreakdown.workHours || 0;
        ...
        // --- Sort caregivers by weighted score ---
    ...
    if (availableCaregivers.length > 0) {
        ...
        var normalizedBestCandidate = normName(bestCandidate.caregiverName);  // ✅ ADD THIS LINE
        ...
        globalAssignedCaregivers[assignmentKey] = normalizedBestCandidate;  // ✅ CHANGED

        // FIXED: Correct hours calculation with one decimal place
        var shiftHours = Math.round(((schedule.endTime - schedule.startTime) / 60) * 10) / 10;
        globalCaregiverHours[normalizedBestCandidate] =  // ✅ CHANGED
            (globalCaregiverHours[normalizedBestCandidate] || 0) + shiftHours;

        clientCaregiverHours[normalizedBestCandidate] =  // ✅ CHANGED
            (clientCaregiverHours[normalizedBestCandidate] || 0) + shiftHours;

        if (!globalCaregiverTimeSlots[normalizedBestCandidate]) {  // ✅ CHANGED
            globalCaregiverTimeSlots[normalizedBestCandidate] = [];
        }

        globalCaregiverTimeSlots[normalizedBestCandidate].push({  // ✅ CHANGED
            clientId: clientId,
            date: dayObj.iso,
            startTime: schedule.startTime,
            endTime: schedule.endTime
        ...
        for (var d2 = 0; d < next7Days.length; d2++) {
            ...
            if (normStr(dayObj.day).toUpperCase() === normStr(dayName).toUpperCase()) {
                ...
                allSchedules[dayObj.iso][clientId].push({
                    ...
                    day: dayName,
                    caregiversRequired: Math.max(1, caregiversRequired) // NEW: Include in schedule data
                });
...
function getClientSchedulesFromAPI(clientId, clientSchedulesData) {
    ...
    for (var i = 0; i < schedulesList.length; i++) {
        ...
        // NEW: Read Caregivers_Required field (defaults to 1 if not specified)
        var caregiversRequired = safeParseNumber(
            safeGetValue(scheduleItem.fields, 'Caregivers_Required.value', 1),
            1
        );
        ...
        schedules.push({
            ...
            caregiversRequired: Math.max(1, caregiversRequired) // NEW: Ensure at least 1
        });
    }
...
var clientLeaveMap = {}; // Indexed by actual client record ID

// FIXED: Handle both array and object with data property
var clientLeavesArray = [];
if (clientLeaves) {
    if (isArray(clientLeaves)) {
        // clientLeaves is directly an array
        clientLeavesArray = clientLeaves;
    } else if (clientLeaves.data && isArray(clientLeaves.data)) {
        // clientLeaves has a data property
        clientLeavesArray = clientLeaves.data;
    }
}

for (var cl_i = 0; cl_i < clientLeavesArray.length; cl_i++) {
    var leave = clientLeavesArray[cl_i];
    if (!leave || !leave.fields) continue;

    // Client_Name.value contains the client ID (not the name)
    ...
    if (leaveStatus === "Approved" && leaveClientId) {
...
// Add debug logging to verify the map is built correctly
result.debug.clientLeaveMapBuilt = {
    totalLeaves: clientLeavesArray.length,
    mappedClients: Object.keys(clientLeaveMap).length,
    clientsWithLeaves: Object.keys(clientLeaveMap),
    leaveDetails: Object.keys(clientLeaveMap).map(function (clientId) {
        return {
            clientId: clientId,
            leavesCount: clientLeaveMap[clientId].length,
            leaves: clientLeaveMap[clientId].map(function (leave) {
                return {
                    startDate: safeGetValue(leave.fields, 'Start_Date.value', ''),
                    endDate: safeGetValue(leave.fields, 'End_Date.value', ''),
                    startTime: safeGetValue(leave.fields, 'Start_Time.value', ''),
                    endTime: safeGetValue(leave.fields, 'End_Time.value', ''),
                    status: safeGetValue(leave.fields, 'Leave_Status.value', '')
                };
            })
        };
    })
...
// Add debug logging to verify the map is built correctly
result.debug.clientLeaveMapBuilt = {
    totalLeaves: clientLeaves && clientLeaves.data ? clientLeaves.data.length : 0,
    mappedClients: Object.keys(clientLeaveMap).length,
    clientsWithLeaves: Object.keys(clientLeaveMap)
};
...
for (var clientIndex = 0; clientIndex < clientsToProcess.length; clientIndex++) {
    ...
    for (var dayIndex = 0; dayIndex < next7Days.length; dayIndex++) {
        ...
        for (var scheduleIndex = 0; scheduleIndex < clientScheduleRows.length; scheduleIndex++) {
            ...
            var scheduleDay = normStr(schedule.day).toUpperCase();
            var currentDay = normStr(dayObj.day).toUpperCase();

            // Check if this schedule's day matches the current day being processed
            if (scheduleDay !== currentDay) {
                continue; // Skip schedules that don't match this day
            }

            // Validate that schedule has valid start/end times
            if (schedule.startTime === null || schedule.endTime === null || schedule.startTime >= schedule.endTime) {
                // Log invalid schedule for debugging
                if (!result.debug.invalidSchedules) {
                    result.debug.invalidSchedules = [];
                }
                result.debug.invalidSchedules.push({
                    clientId: clientId,
                    clientName: clientName,
                    day: dayObj.day,
                    date: dayObj.iso,
                    reason: 'invalid_time_format',
                    startTimeStr: schedule.startTimeStr || 'missing',
                    endTimeStr: schedule.endTimeStr || 'missing'
                });
                continue; // Skip invalid schedules
            }

            // ========== NEW: CHECK CLIENT SUBSCRIPTION VALIDITY ==========
            if (!isClientSubscriptionValid(clientData, dayObj.iso)) {
                // Log skipped schedule due to invalid subscription
                if (!result.debug.skippedSchedules) {
                    result.debug.skippedSchedules = [];
                }
                result.debug.skippedSchedules.push({
                    clientId: clientId,
                    clientName: clientName,
                    day: dayObj.day,
                    date: dayObj.iso,
                    time: schedule.startTimeStr + '-' + schedule.endTimeStr,
                    reason: 'client_subscription_invalid',
                    effectiveFrom: safeGetValue(clientData.fields, 'Effective_From.value', 'N/A'),
                    effectiveTo: safeGetValue(clientData.fields, 'Effective_To.value', 'N/A')
                });
                continue; // Skip this schedule entirely
            }

            // ========== NEW: CHECK IF CLIENT IS ON LEAVE ==========
            if (isClientOnLeave(clientId, dayObj.iso, schedule.startTime, schedule.endTime, clientLeaves)) {
                // Log skipped schedule due to client leave
                if (!result.debug.skippedSchedules) {
                    result.debug.skippedSchedules = [];
                }
                result.debug.skippedSchedules.push({
                    ...
                    date: dayObj.iso,
                    time: schedule.startTimeStr + '-' + schedule.endTimeStr,
                    reason: 'client_on_leave',
                    leaveDetails: clientLeaveMap[clientId] || []
                });
                continue; // Skip this schedule entirely
            }
            // ========== END NEW CHECKS ==========
            ...
            for (var caregiverSlot = 0; caregiverSlot < caregiversRequired; caregiverSlot++) {
                var assignmentResult = assignCaregiverToSchedule(
                    ...
                    caregiverSlot,  // Pass the actual slot index (0, 1, 2, ...)
                    assignedCaregiversForThisSchedule  // Pass the list of already assigned caregivers
                );



                // Add debugging for null assignment results
                if (!assignmentResult) {
                    if (!result.debug.nullAssignments) {
                        result.debug.nullAssignments = [];
                    }
                    result.debug.nullAssignments.push({
                        clientId: clientId,
                        clientName: clientName,
                        date: dayObj.iso,
                        day: dayObj.day,
                        time: schedule.startTimeStr + '-' + schedule.endTimeStr,
                        caregiverSlot: caregiverSlot + 1,
                        caregiversRequired: caregiversRequired,
                        reason: 'assignment_function_returned_null'
                    });
                }
                ...
                if (isAvailable && assignedCaregiver !== "Unassigned" &&
                    ...
                    for (var alt = 0; alt < availableList.length; alt++) {
                        var altCaregiverObj = availableList[alt];  // FIX: Get whole object
                        var altCaregiver = altCaregiverObj.caregiverName;

                        // Skip if already assigned to this schedule
                        if (assignedCaregiversForThisSchedule.indexOf(altCaregiver) === -1) {
                            // Double-check availability before assigning
                            var isAltAvailable = isCaregiverAvailable(
                                altCaregiver,
                                clientId,
                                dayObj.iso,
                                schedule.startTime,
                            ...
                            if (isAltAvailable) {
                                // Found an alternative caregiver not yet assigned
                                assignedCaregiver = altCaregiver;
                                assignmentResult.assignedCaregiver = altCaregiver;

                                // FIX: Update weighted breakdown to match the alternative caregiver
                                assignmentResult.assignedCaregiverWeightedTotal = altCaregiverObj.weightedTotalScore || null;
                                assignmentResult.assignedCaregiverWeightedBreakdown = altCaregiverObj.weightedBreakdown || null;
                                assignmentResult.assignedCaregiverScore = altCaregiverObj.weightedTotalScore || null;
                                ...
                                var normalizedAltCaregiver = normName(altCaregiver);
                                globalAssignedCaregivers[assignmentKey] = normalizedAltCaregiver;

                                var shiftHours = Math.round(((schedule.endTime - schedule.startTime) / 60) * 10) / 10;
                                globalCaregiverHours[normalizedAltCaregiver] =
                                    (globalCaregiverHours[normalizedAltCaregiver] || 0) + shiftHours;

                                if (!globalCaregiverTimeSlots[normalizedAltCaregiver]) {
                                    globalCaregiverTimeSlots[normalizedAltCaregiver] = [];
                                }

                                globalCaregiverTimeSlots[normalizedAltCaregiver].push({
                                    clientId: clientId,
                                    date: dayObj.iso,
                                    startTime: schedule.startTime,
                                    endTime: schedule.endTime
                    ...
                    if (!alternativeFound) {
                        ...
                        // Add debug info
                        if (!result.debug.multiCaregiverShortage) {
                            result.debug.multiCaregiverShortage = [];
                        }
                        result.debug.multiCaregiverShortage.push({
                            clientId: clientId,
                            clientName: clientName,
                            date: dayObj.iso,
                            time: schedule.startTimeStr + '-' + schedule.endTimeStr,
                            required: caregiversRequired,
                            assigned: caregiverSlot,
                            reason: 'insufficient_available_caregivers'
                        });
                    }
                }
...
for (var c = 0; c < result.allClientAssignments.length; c++) {
    ...
    for (var s = 0; s < clientAssignment.scheduledServices.length; s++) {
        ...
        if (service.caregiverName && service.caregiverName !== "Unassigned") {
            // Store a canonical version (first occurrence) to avoid duplicates
            var normalizedName = normName(service.caregiverName);
            if (!uniqueCaregiversUsed[normalizedName]) {
                uniqueCaregiversUsed[normalizedName] = service.caregiverName; // Store the display name
            }
        }
...
result.globalSummary = {
    ...
    caregiverNames: Object.keys(uniqueCaregiversUsed).map(function (key) {
        return uniqueCaregiversUsed[key]; // Return the display names
    }),
...
for (var c = 0; c < result.allClientAssignments.length; c++) {
    ...
    for (var s = 0; s < clientAssignment.scheduledServices.length; s++) {
        var service = clientAssignment.scheduledServices[s];
        if (service.caregiverName && service.caregiverName !== "Unassigned") {
            // Use normalized name for tracking to avoid duplicates
            var normalizedName = normName(service.caregiverName);

            // But find the properly formatted name for display
            var displayName = service.caregiverName;

            caregiverUtilization[displayName] = (caregiverUtilization[displayName] || 0) + 1;
