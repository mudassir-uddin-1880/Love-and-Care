// Working code With Dynamic Setting changes BUt leavs Issues
...
function weeklyDistributionCheck(employeeData, candidateShiftHours, settingsSnapshot, debugFlag) {
    ...
    // Calculate from current assignments in this scheduling run
    for (var assignmentKey in globalAssignedCaregivers) {
        if (globalAssignedCaregivers.hasOwnProperty(assignmentKey)) {
            ...
            if (normName(assignedCaregiver) === normName(employeeName)) {
                // Extract hours from assignment key or use standard shift duration
                currentWeeklyHours += candidateShiftHours; // This is simplified - could be improved
            }
        }
    }
...
function assignCaregiverToSchedule(clientData, dayObj, schedule, employeesDetails,
    ...
    var allEvaluatedCaregivers = []; // Track ALL caregivers for debugging
    ...
    for (var ac = 0; ac < allNames.length; ac++) {
        ...
        if (!isBasicAvailable) {
            ...
            allEvaluatedCaregivers.push(candidateResult);
            continue;
        }

        // Step 2: Check if on leave
        var isOnLeave = isCaregiverOnLeave(cand, dayObj.iso, leavesData);
        if (isOnLeave) {
        ...
        // Step 6: Check Max Weekly Hours (HARD CONSTRAINT)
    ...
    for (var i = 0; i < availableCaregivers.length; i++) {
        availableCaregivers[i].rank = i + 1;
        if (i === 0) {
            availableCaregivers[i].status = availableCaregivers[i].status.indexOf('Fallback') !== -1 ?
                'Selected (Fallback)' : 'Selected';
        } else {
            availableCaregivers[i].status = availableCaregivers[i].status.indexOf('Fallback') !== -1 ?
                'Available (Fallback) - Rank ' + (i + 1) : 'Available - Rank ' + (i + 1);
        }
    }

    // Include ALL evaluated caregivers for complete visibility
    var allCaregiversWithRanking = [];

    // Add available caregivers first
    for (var i = 0; i < availableCaregivers.length; i++) {
        allCaregiversWithRanking.push(availableCaregivers[i]);
    }

    // Add non-available caregivers for reference
    for (var i = 0; i < allEvaluatedCaregivers.length; i++) {
        var evaluated = allEvaluatedCaregivers[i];
        var alreadyAdded = false;
        for (var j = 0; j < availableCaregivers.length; j++) {
            if (availableCaregivers[j].caregiverName === evaluated.caregiverName) {
                alreadyAdded = true;
                break;
            }
        }
        if (!alreadyAdded) {
            evaluated.rank = 999; // Mark as not ranked
            allCaregiversWithRanking.push(evaluated);
        }
    }

    // Set the result - show ALL caregivers
    res.availableCaregiversList = allCaregiversWithRanking;
...
function checkScheduleConflicts(caregiverName, targetClientId, targetStartTime, targetEndTime, targetDate, allSchedules, assignedCaregivers) {
    var conflicts = [];

    // Check existing assignments for this caregiver on the same day
    for (var key in assignedCaregivers) {
        if (assignedCaregivers.hasOwnProperty(key) && assignedCaregivers[key] === caregiverName) {
            var keyParts = key.split('_');
            if (keyParts.length >= 3 && keyParts[1] === targetDate) {
                var existingStartTime = parseInt(keyParts[2], 10);
                // Simple conflict check - you may need to enhance this based on your schedule structure
                if (timeOverlap(targetStartTime, targetEndTime, existingStartTime, existingStartTime + 60)) {
                    conflicts.push({
