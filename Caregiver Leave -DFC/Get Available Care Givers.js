...
function checkGenderPreference(caregiverName, clientData, employeesDetails, isMandatory) {
    ...
    var rawPref = safeGetValue(clientFields, 'Gender_Preference.value', '');
    var isStrict = normStr(safeGetValue(clientFields, 'Gender_Preference_Strict.value', '')).toLowerCase() === 'yes';

    // Normalize preference: treat "any", "either", or empty as no preference
    var prefList = parseList(rawPref).filter(function (p) {
        var v = p.toLowerCase();
        return v && v !== 'any' && v !== 'either';
    });

    // Warn if strict is set but no valid preference
    if (isStrict && prefList.length === 0) {
        if (typeof result !== 'undefined') {
            result.debug = result.debug || {};
            result.debug.genderStrictNoPref = result.debug.genderStrictNoPref || [];
            result.debug.genderStrictNoPref.push({
                caregiverName: caregiverName,
                clientId: clientData.id || '',
                rawPref: rawPref
            });
        }
        return { passes: true, reason: 'strict_but_no_gender_preference', scoreBoost: 0 };
    }

    // No preference at all
    if (prefList.length === 0) {
        return { passes: true, reason: 'no_gender_preference', scoreBoost: 0 };
    }
    ...
    // Warn if caregiver gender is missing
    if (!caregiverGender && typeof result !== 'undefined') {
        result.debug = result.debug || {};
        result.debug.caregiverMissingGender = result.debug.caregiverMissingGender || [];
        result.debug.caregiverMissingGender.push({
            caregiverName: caregiverName,
            clientId: clientData.id || ''
        });
    }

    // Fuzzy match helper using compatible string methods
    function fuzzyMatch(pref, actual) {
        if (!pref || !actual) return false;
        if (pref === actual) return true;
        // Replace startsWith with substring comparison
        if (pref.length >= 2 && actual.length >= 2) {
            var actualPrefix = actual.substring(0, 2);
            return pref.substring(0, 2) === actualPrefix;
        }
        return false;
    }

    // Controlled vocabulary
    var allowedGenders = ['male', 'female', 'non-binary', 'other'];
    var matches = false;

    // Replace some() with traditional loop
    for (var i = 0; i < prefList.length; i++) {
        var pref = prefList[i];
        var normPref = pref.toLowerCase();
        if (allowedGenders.indexOf(normPref) !== -1) {
            if (normPref === caregiverGender) {
                matches = true;
                break;
            }
        } else {
            if (fuzzyMatch(normPref, caregiverGender)) {
                matches = true;
                break;
            }
        }
    ...
    var shouldEnforce = isStrict;

    return {
        passes: shouldEnforce ? matches : true,
        reason: matches ? 'gender_match' : 'gender_mismatch',
        scoreBoost: matches ? 5 : 0
    };
}
...
function checkPhysicalCapability(caregiverName, clientData, employeesDetails, isMandatory) {
    ...
    if (!weightClassMatch || !lbsMatch) {
        if (typeof result !== 'undefined') {
            result.debug = result.debug || {};
            result.debug.physicalCapabilityRejects = result.debug.physicalCapabilityRejects || [];
...
function checkSkillsRequirement(caregiverName, clientData, employeesDetails, isMandatory) {
    var clientFields = clientData.fields || {};
    var rawSkills = safeGetValue(clientFields, 'Skills_Preferences.value', '');
    var requiredSkills = normalizeSkillList(rawSkills);

    // Controlled vocabulary for skills
    var allowedSkills = [
        'cpr', 'first aid', 'hoyer lift', 'medication administration', 'dementia care', 'alzheimers care',
        'wound care', 'catheter care', 'feeding', 'bathing', 'mobility assistance', 'vital signs'
    ];

    // Filter out "any" or "either"
    var filteredSkills = requiredSkills.filter(function (s) {
        var v = s.toLowerCase();
        return v && v !== 'any' && v !== 'either';
    });

    // Warn if mandatory but no valid preference
    if (isMandatory && filteredSkills.length === 0) {
        if (typeof result !== 'undefined') {
            result.debug = result.debug || {};
            result.debug.skillsStrictNoPref = result.debug.skillsStrictNoPref || [];
            result.debug.skillsStrictNoPref.push({
                caregiverName: caregiverName,
                clientId: clientData.id || '',
                rawSkills: rawSkills
            });
        }
        return { passes: true, reason: 'strict_but_no_skills_preference', scoreBoost: 0, matchCount: 0, totalRequired: 0 };
    }

    if (filteredSkills.length === 0) {
        return { passes: true, reason: 'no_skills_requirement', scoreBoost: 0, matchCount: 0, totalRequired: 0 };
    }

    var empRecord = getEmployeeRecordByName(caregiverName, employeesDetails);
    if (!empRecord || !empRecord.fields) {
        return { passes: false, reason: 'employee_not_found', scoreBoost: 0, matchCount: 0, totalRequired: filteredSkills.length };
    }

    var caregiverSkills = normalizeSkillList(safeGetValue(empRecord.fields, 'Skill_Type.value', ''));
    var matchCount = 0;

    // Define fuzzyMatch function using compatible string methods
    function fuzzyMatchSkill(pref, actual) {
        if (!pref || !actual) return false;
        if (pref === actual) return true;
        // Replace startsWith with substring comparison
        if (pref.length >= 2 && actual.length >= 2) {
            var actualPrefix = actual.substring(0, 2);
            var prefPrefix = pref.substring(0, 2);
            return prefPrefix === actualPrefix;
        }
        return false;
    }

    // Log unusual skill values
    for (var i = 0; i < filteredSkills.length; i++) {
        var skill = filteredSkills[i];
        if (allowedSkills.indexOf(skill) === -1 && typeof result !== 'undefined') {
            result.debug = result.debug || {};
            result.debug.unusualSkillValues = result.debug.unusualSkillValues || [];
            result.debug.unusualSkillValues.push({
                clientId: clientData.id || '',
                caregiverName: caregiverName,
                skill: skill
            });
    ...
    // Match with controlled vocabulary and fuzzy matching
    for (var j = 0; j < filteredSkills.length; j++) {
        var reqSkill = filteredSkills[j];
        var found = false;

        for (var k = 0; k < caregiverSkills.length; k++) {
            var cgSkill = caregiverSkills[k];
            var normReq = reqSkill.toLowerCase();
            var normCg = cgSkill.toLowerCase();

            if (allowedSkills.indexOf(normReq) !== -1) {
                if (normReq === normCg) {
                    found = true;
                    break;
                }
            } else {
                if (fuzzyMatchSkill(normReq, normCg)) {
                    found = true;
                    break;
                }
            }
    ...
    var allMatch = matchCount === filteredSkills.length;
    ...
    return {
        ...
        scoreBoost: scoreBoost,
        matchCount: matchCount,
        totalRequired: filteredSkills.length
    };
}
...
function checkClientTypeCompatibility(caregiverName, clientData, employeesDetails, isMandatory) {
    ...
    if (typeof result !== 'undefined') {
        result.debug = result.debug || {};
        result.debug.clientTypeChecks = result.debug.clientTypeChecks || [];
    }

    if (!clientData || !clientData.fields) {
        return { passes: !isMandatory, reason: 'client_data_missing', scoreBoost: 0 };
    }

    // Extract client type with facility name override
    var clientType = safeGetValue(clientData.fields, 'Client_Type.value', '');
    var facilityName = safeGetValue(clientData.fields, 'Facility_Name.value', '');

    var clientTypeNorm = normStr(clientType).toLowerCase();
    if (normStr(facilityName) !== '') {
        clientTypeNorm = 'facility'; // Override if facility name exists
    }

    // Default to 'private' if empty
    if (!clientTypeNorm) {
        clientTypeNorm = 'private';
    }

    var empRecord = getEmployeeRecordByName(caregiverName, employeesDetails);
    if (!empRecord || !empRecord.fields) {
        return { passes: !isMandatory, reason: 'employee_not_found', scoreBoost: 0 };
    }

    // More flexible affirmative check
    function isAffirmative(value) {
        var norm = normStr(value).toLowerCase();
        return norm === 'yes' || norm === 'y' || norm === 'true' || norm === '1';
    }

    var facilityEligible = isAffirmative(safeGetValue(empRecord.fields, 'Facility.value', ''));
    var privateEligible = isAffirmative(safeGetValue(empRecord.fields, 'Private.value', ''));

    // Debug logging
    if (typeof result !== 'undefined') {
        result.debug.clientTypeChecks.push({
            caregiverName: caregiverName,
            clientType: clientTypeNorm,
            facilityEligible: facilityEligible,
            privateEligible: privateEligible
        });
    }
    ...
    // Check eligibility - both types first
    if (facilityEligible && privateEligible) {
        isCompatible = true;
        reason = 'caregiver_eligible_for_both_types';
    } else if (clientTypeNorm === 'facility' && facilityEligible) {
        isCompatible = true;
    ...
    var scoreBoost = isCompatible ? 3 : 0;
...
function assignCaregiverToSchedule(clientData, dayObj, schedule, employeesDetails,
    ...
    if (availableCaregivers.length === 0) {
        ...
        if ((hasStrictGenderReq && genderPref) || isPhysicalMandatory) {
            res.finalAvailabilityIssue = isPhysicalMandatory
                ? 'no_caregiver_available_due_to_physical_capability'
                : 'no_caregiver_available_due_to_strict_gender_preference';
            res.debugReasons.push({
                priority: isPhysicalMandatory ? 'Caregiver Physical Capability' : 'Gender Preference',
                allowed: false,
                scoreAdjustment: 0,
                reason: isPhysicalMandatory
                    ? 'No caregiver matches required weight class (mandatory in settings)'
...
// updated Code by John - For Leave reassignments 

function handleCaregiverLeaveCancellationFromSchedules(caregiverLeaveData, getscheduledlist, employeesDetails,
    leavesData, allClientsScheduleData, settingsTableData) {

    var reAssignmentResult = {
        caregiverName: '',
        caregiverEmployeeId: '',
        caregiverDetails: {},
        leavePeriod: { startDate: '', endDate: '', startTime: '', endTime: '' },
        affectedSchedules: {
            total: 0,
            directAssignments: [],
            addedToAlternatives: [],
        ...
        summary: {
            ...
            directlyAssigned: 0,
            addedAsAlternative: 0,
            notApplicableCount: 0,
            message: ''
        }
    };

    if (!caregiverLeaveData || !caregiverLeaveData.fields) {
        reAssignmentResult.summary.message = 'Invalid leave data provided';
        return reAssignmentResult;
    }

    var leaveFields = caregiverLeaveData.fields;
    var caregiverNameFromLeave = safeGetValue(leaveFields, 'Caregiver.value', '');
    ...
    reAssignmentResult.caregiverName = caregiverNameFromLeave;
    reAssignmentResult.leavePeriod = {
    ...
    var employeeRecord = getEmployeeRecordByName(caregiverNameFromLeave, employeesDetails);
    if (!employeeRecord) {
        reAssignmentResult.summary.message = 'Caregiver not found in employee records: ' + caregiverNameFromLeave;
        return reAssignmentResult;
    }

    reAssignmentResult.caregiverEmployeeId = employeeRecord.id || '';

    var empFields = employeeRecord.fields || {};
    reAssignmentResult.caregiverDetails = {
        gender: safeGetValue(empFields, 'Gender.value', ''),
        languages: normalizeSkillList(safeGetValue(empFields, 'Language.value', '')),
        skills: normalizeSkillList(safeGetValue(empFields, 'Skill_Type.value', '')),
        weightClass: safeGetValue(empFields, 'Weight_Class.value', ''),
        physicalCapability: safeParseNumber(safeGetValue(empFields, 'Physical_Capability_lbs.value', 0), 0),
        privateEligible: normStr(safeGetValue(empFields, 'Private.value', '')).toLowerCase() === 'yes',
        facilityEligible: normStr(safeGetValue(empFields, 'Facility.value', '')).toLowerCase() === 'yes',
        hasDriverLicense: normStr(safeGetValue(empFields, 'Driver_License_.value', '')).toLowerCase() === 'yes',
        hasCar: normStr(safeGetValue(empFields, 'Has_Car_.value', '')).toLowerCase() === 'yes',
        personality: parseList(safeGetValue(empFields, 'Personality_Match.value', ''))
    };

    var leaveStartMin = parseTime(leaveStartTime);
    var leaveEndMin = parseTime(leaveEndTime);

    if (!isArray(getscheduledlist)) {
        reAssignmentResult.summary.message = 'No schedules list provided';
        return reAssignmentResult;
    }

    for (var i = 0; i < getscheduledlist.length; i++) {
        var schedule = getscheduledlist[i];
        if (!schedule || !schedule.fields) continue;

        var scheduleFields = schedule.fields;
        var clientName = safeGetValue(scheduleFields, 'Client_Name.value', '');
        var scheduleStatus = safeGetValue(scheduleFields, 'Scheduling_Status.value', '');
        var scheduleDate = safeGetValue(scheduleFields, 'Schedule_Start_Date.value', '');
        var scheduleStartTimeStr = safeGetValue(scheduleFields, 'Schedule_Start_Time.value', '');
        var scheduleEndTimeStr = safeGetValue(scheduleFields, 'Schedule_End_Time.value', '');
        var expectedCaregiver = safeGetValue(scheduleFields, 'Expected_Caregiver.value', '');
        var actualCaregiver = safeGetValue(scheduleFields, 'Actual_Caregiver.value', '');

        if (scheduleDate < leaveStartDate || scheduleDate > leaveEndDate) continue;

        var scheduleStartTime = parseTime(scheduleStartTimeStr);
        var scheduleEndTime = parseTime(scheduleEndTimeStr);
        if (scheduleStartTime === null || scheduleEndTime === null) continue;

        var timeOverlaps = true;
        if (leaveStartMin !== null && leaveEndMin !== null) {
            timeOverlaps = timeOverlap(scheduleStartTime, scheduleEndTime, leaveStartMin, leaveEndMin);
        }
        if (!timeOverlaps) continue;

        reAssignmentResult.affectedSchedules.total++;

        var clientData = null;
        for (var cd = 0; cd < allClientsScheduleData.length; cd++) {
            var client = allClientsScheduleData[cd];
            var clientFullName = safeGetValue(client, 'fields.Client_Full_Name.value', '');
            if (normName(clientFullName) === normName(clientName)) {
                clientData = client;
                break;
            }
        }
        ...
        if (!clientData) {
            reAssignmentResult.affectedSchedules.notApplicable.push({
                scheduleId: schedule.id,
                clientName: clientName,
                date: scheduleDate,
                serviceTime: scheduleStartTimeStr + ' - ' + scheduleEndTimeStr,
                reason: 'client_data_not_found',
                currentStatus: scheduleStatus,
                criteriaChecked: ['client_lookup'],
                failedAt: 'client_lookup'
            });
            continue;
        }
        ...
        var clientFields = clientData.fields || {};
        var isOpenShift = (
            scheduleStatus === 'Open Shift' ||
            scheduleStatus === 'Canceled By Caregiver' ||
            scheduleStatus === 'Cancelled by Client' ||
            (!expectedCaregiver && !actualCaregiver)
        );

        var dateParts = scheduleDate.split('-');
        var y = parseInt(dateParts[0], 10), m = parseInt(dateParts[1], 10), d = parseInt(dateParts[2], 10);
        var dayOfWeekNum = (function (yy, mm, dd) {
            var t = [0, 3, 2, 5, 0, 3, 5, 1, 4, 6, 2, 4];
            if (mm < 3) yy -= 1;
            return (yy + Math.floor(yy / 4) - Math.floor(yy / 100) + Math.floor(yy / 400) + t[mm - 1] + dd) % 7;
        })(y, m, d);
        var dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
        var dayName = dayNames[dayOfWeekNum];

        // EARLY EXIT PATTERN - Stop at first hard constraint failure
        var criteriaResult = {
            clientRequirements: {},
            caregiverCapabilities: {},
            hardConstraints: {},
            softPreferences: {},
            failedAt: null,
            passedAll: false
        ...
        // 1. TIME SLOT AVAILABILITY (Hard)
        var isAvailableForTimeSlot = isCaregiverAvailableForSchedule(
            caregiverNameFromLeave, dayName, scheduleStartTime, scheduleEndTime, employeesDetails
        );

        criteriaResult.hardConstraints.timeSlotAvailability = {
            required: 'Available for ' + dayName + ' ' + scheduleStartTimeStr + '-' + scheduleEndTimeStr,
            caregiverHas: isAvailableForTimeSlot ? 'Available' : 'Not Available',
            passed: isAvailableForTimeSlot
        };

        if (!isAvailableForTimeSlot) {
            criteriaResult.failedAt = 'time_slot_availability';
            reAssignmentResult.affectedSchedules.notApplicable.push({
                scheduleId: schedule.id,
                clientName: clientName,
                clientId: clientId,
        ...
        // 2. OTHER LEAVE CONFLICTS (Hard)
        var hasOtherLeaveConflict = false;
        for (var l = 0; l < leavesData.length; l++) {
            var otherLeave = leavesData[l];
            if (!otherLeave || !otherLeave.fields || otherLeave.id === caregiverLeaveData.id) continue;

            var otherLeaveCaregiver = safeGetValue(otherLeave.fields, 'Caregiver.value', '');
            var otherLeaveStatus = safeGetValue(otherLeave.fields, 'Leave_Status.value', '');

            if (normName(otherLeaveCaregiver) !== normName(caregiverNameFromLeave) || otherLeaveStatus !== 'Approved') continue;

            var otherStartDate = safeGetValue(otherLeave.fields, 'Start_Date.value', '');
            var otherEndDate = safeGetValue(otherLeave.fields, 'End_Date.value', '');

            if (scheduleDate >= otherStartDate && scheduleDate <= otherEndDate) {
                hasOtherLeaveConflict = true;
                break;
            }
        }

        criteriaResult.hardConstraints.leaveConflicts = {
            required: 'No approved leaves on this date',
            caregiverHas: hasOtherLeaveConflict ? 'Has conflicting leave' : 'No conflicts',
            passed: !hasOtherLeaveConflict
        };

        if (hasOtherLeaveConflict) {
            criteriaResult.failedAt = 'other_leave_conflict';
            reAssignmentResult.affectedSchedules.notApplicable.push({
                scheduleId: schedule.id,
                clientName: clientName,
                clientId: clientId,
                ...
                day: dayName,
                serviceTime: scheduleStartTimeStr + ' - ' + scheduleEndTimeStr,
                reason: 'has_other_approved_leave',
                currentStatus: scheduleStatus,
                criteriaResult: criteriaResult
            });
            continue;
        ...
        // 3. CLIENT TYPE COMPATIBILITY (Hard)
        var clientType = safeGetValue(clientFields, 'Client_Type.value', '');
        var clientTypeNorm = normStr(clientType).toLowerCase();
        var clientTypeCompatible = false;

        if (clientTypeNorm === 'facility' && reAssignmentResult.caregiverDetails.facilityEligible) {
            clientTypeCompatible = true;
        } else if (clientTypeNorm === 'private' && reAssignmentResult.caregiverDetails.privateEligible) {
            clientTypeCompatible = true;
        }

        criteriaResult.clientRequirements.clientType = clientType;
        criteriaResult.caregiverCapabilities.eligibleFor = [];
        if (reAssignmentResult.caregiverDetails.privateEligible) criteriaResult.caregiverCapabilities.eligibleFor.push('Private');
        if (reAssignmentResult.caregiverDetails.facilityEligible) criteriaResult.caregiverCapabilities.eligibleFor.push('Facility');

        criteriaResult.hardConstraints.clientTypeCompatibility = {
            required: clientType + ' client eligibility',
            caregiverHas: criteriaResult.caregiverCapabilities.eligibleFor.join(', ') || 'None',
            passed: clientTypeCompatible
        };

        if (!clientTypeCompatible) {
            criteriaResult.failedAt = 'client_type_incompatibility';
            reAssignmentResult.affectedSchedules.notApplicable.push({
                scheduleId: schedule.id,
                clientName: clientName,
                clientId: clientId,
                ...
                day: dayName,
                serviceTime: scheduleStartTimeStr + ' - ' + scheduleEndTimeStr,
                reason: 'client_type_incompatible',
                currentStatus: scheduleStatus,
                criteriaResult: criteriaResult
            });
            continue;
        ...
        // 4. GENDER PREFERENCE (Hard if strict)
        var genderPref = safeGetValue(clientFields, 'Gender_Preference.value', '');
        var isStrictGender = normStr(safeGetValue(clientFields, 'Gender_Preference_Strict.value', '')).toLowerCase() === 'yes';
        var genderMatch = !genderPref || normStr(genderPref).toLowerCase() === normStr(reAssignmentResult.caregiverDetails.gender).toLowerCase();

        criteriaResult.clientRequirements.gender = genderPref || 'Any';
        criteriaResult.clientRequirements.genderStrict = isStrictGender;
        criteriaResult.caregiverCapabilities.gender = reAssignmentResult.caregiverDetails.gender;

        criteriaResult.hardConstraints.genderMatch = {
            required: genderPref ? (genderPref + (isStrictGender ? ' (Strict)' : ' (Preferred)')) : 'Any',
            caregiverHas: reAssignmentResult.caregiverDetails.gender,
            passed: !isStrictGender || genderMatch,
            isHardConstraint: isStrictGender
        };

        if (isStrictGender && !genderMatch) {
            criteriaResult.failedAt = 'gender_strict_mismatch';
            reAssignmentResult.affectedSchedules.notApplicable.push({
                scheduleId: schedule.id,
                clientName: clientName,
                clientId: clientId,
        ...
        // 5. WEIGHT CLASS / PHYSICAL CAPABILITY (Hard)
        var clientWeightClass = normStr(safeGetValue(clientFields, 'Weight_Class.value', '')).toLowerCase();
        var caregiverWeightClass = normStr(safeGetValue(empFields, 'Weight_Class.value', '')).toLowerCase();
        var weightClassMatch = true;
        ...
        criteriaResult.clientRequirements.weightClass = clientWeightClass || 'Any';
        criteriaResult.caregiverCapabilities.weightClass = caregiverWeightClass;
        criteriaResult.caregiverCapabilities.physicalCapabilityLbs = reAssignmentResult.caregiverDetails.physicalCapability;

        criteriaResult.hardConstraints.physicalCapability = {
            required: clientWeightClass ? (clientWeightClass.charAt(0).toUpperCase() + clientWeightClass.slice(1) + ' weight class') : 'Any',
            caregiverHas: caregiverWeightClass ? (caregiverWeightClass.charAt(0).toUpperCase() + caregiverWeightClass.slice(1) + ' (' + reAssignmentResult.caregiverDetails.physicalCapability + ' lbs)') : 'Not specified',
            passed: weightClassMatch
        };

        if (!weightClassMatch && clientWeightClass) {
            criteriaResult.failedAt = 'weight_class_mismatch';
            reAssignmentResult.affectedSchedules.notApplicable.push({
                scheduleId: schedule.id,
                clientName: clientName,
                clientId: clientId,
                ...
                day: dayName,
                serviceTime: scheduleStartTimeStr + ' - ' + scheduleEndTimeStr,
                reason: 'physical_capability_insufficient',
                currentStatus: scheduleStatus,
                criteriaResult: criteriaResult
            });
            continue;
        ...
        var blockList = parseList(safeGetValue(clientFields, 'Caregiver_Block_List.value', ''));
        var isBlocked = false;
        var caregiverNameNorm = normName(caregiverNameFromLeave);

        for (var bl = 0; bl < blockList.length; bl++) {
            if (blockList[bl] === caregiverNameNorm) {
                isBlocked = true;
                break;
            }
        }

        criteriaResult.hardConstraints.blocklistCheck = {
            required: 'Not on client blocklist',
            caregiverHas: isBlocked ? 'BLOCKED' : 'Not blocked',
            passed: !isBlocked
        };

        if (isBlocked) {
            criteriaResult.failedAt = 'blocklist';
            reAssignmentResult.affectedSchedules.notApplicable.push({
                scheduleId: schedule.id,
                clientName: clientName,
                clientId: clientId,
        ...
        // SOFT PREFERENCES (Don't block assignment)
        var clientLanguages = normalizeSkillList(safeGetValue(clientFields, 'Language_Preferences.value', ''));
        var caregiverLanguages = reAssignmentResult.caregiverDetails.languages;
        var languageMatches = 0;
        for (var li = 0; li < clientLanguages.length; li++) {
            if (caregiverLanguages.indexOf(clientLanguages[li]) !== -1) languageMatches++;
        }

        criteriaResult.clientRequirements.languages = clientLanguages;
        criteriaResult.caregiverCapabilities.languages = caregiverLanguages;
        criteriaResult.softPreferences.languages = {
            required: clientLanguages.length > 0 ? clientLanguages.join(', ') : 'Any',
            caregiverHas: caregiverLanguages.join(', '),
            matchCount: languageMatches,
            totalRequired: clientLanguages.length,
            matchPercentage: clientLanguages.length > 0 ? Math.round((languageMatches / clientLanguages.length) * 100) + '%' : 'N/A'
        };

        var clientSkills = normalizeSkillList(safeGetValue(clientFields, 'Skills_Preferences.value', ''));
        var caregiverSkills = reAssignmentResult.caregiverDetails.skills;
        var skillMatches = 0;
        for (var si = 0; si < clientSkills.length; si++) {
            if (caregiverSkills.indexOf(clientSkills[si]) !== -1) skillMatches++;
        }

        criteriaResult.clientRequirements.skills = clientSkills;
        criteriaResult.caregiverCapabilities.skills = caregiverSkills;
        criteriaResult.softPreferences.skills = {
            required: clientSkills.length > 0 ? clientSkills.join(', ') : 'Any',
            caregiverHas: caregiverSkills.join(', '),
            matchCount: skillMatches,
            totalRequired: clientSkills.length,
            matchPercentage: clientSkills.length > 0 ? Math.round((skillMatches / clientSkills.length) * 100) + '%' : 'N/A'
        };

        var transportNeeded = normStr(safeGetValue(clientFields, 'Transportation_Needed_.value', '')).toLowerCase() === 'yes';
        var canProvideTransport = reAssignmentResult.caregiverDetails.hasDriverLicense && reAssignmentResult.caregiverDetails.hasCar;

        criteriaResult.clientRequirements.transportation = transportNeeded;
        criteriaResult.caregiverCapabilities.transportation = {
            hasLicense: reAssignmentResult.caregiverDetails.hasDriverLicense,
            hasCar: reAssignmentResult.caregiverDetails.hasCar,
            canProvide: canProvideTransport
        };
        criteriaResult.softPreferences.transportation = {
            required: transportNeeded ? 'Yes (preferred)' : 'Not required',
            caregiverHas: canProvideTransport ? 'Can provide' : 'Cannot provide'
        };

        var candidateShiftHours = Math.round(((scheduleEndTime - scheduleStartTime) / 60) * 10) / 10;

        // PASSED ALL CHECKS
        criteriaResult.passedAll = true;

        if (isOpenShift) {
            reAssignmentResult.affectedSchedules.directAssignments.push({
                scheduleId: schedule.id,
                clientName: clientName,
                clientId: clientId,
                date: scheduleDate,
                day: dayName,
                serviceTime: scheduleStartTimeStr + ' - ' + scheduleEndTimeStr,
                hours: candidateShiftHours,
                action: 'direct_assignment',
                previousStatus: scheduleStatus,
                caregiverQBID: getCaregiverQBID(caregiverNameFromLeave, employeesDetails),
                criteriaResult: criteriaResult
            });
        } else {
            var newEntry = caregiverNameFromLeave + ' - Eligible';
            reAssignmentResult.affectedSchedules.addedToAlternatives.push({
                scheduleId: schedule.id,
                clientName: clientName,
                clientId: clientId,
                date: scheduleDate,
                day: dayName,
                serviceTime: scheduleStartTimeStr + ' - ' + scheduleEndTimeStr,
                currentCaregiver: expectedCaregiver || actualCaregiver,
                alternativeEntry: newEntry,
                action: 'added_to_alternatives',
                previousStatus: scheduleStatus,
                criteriaResult: criteriaResult
            });
        }
    ...
    // Summary
    reAssignmentResult.summary.totalSchedulesFound = reAssignmentResult.affectedSchedules.total;
    reAssignmentResult.summary.directlyAssigned = reAssignmentResult.affectedSchedules.directAssignments.length;
    reAssignmentResult.summary.addedAsAlternative = reAssignmentResult.affectedSchedules.addedToAlternatives.length;
    reAssignmentResult.summary.notApplicableCount = reAssignmentResult.affectedSchedules.notApplicable.length;

    if (reAssignmentResult.summary.totalSchedulesFound === 0) {
        reAssignmentResult.summary.message = 'No schedules found during leave period (' + leaveStartDate + ' to ' + leaveEndDate + ')';
    } else if (reAssignmentResult.summary.directlyAssigned === 0 && reAssignmentResult.summary.addedAsAlternative === 0) {
        reAssignmentResult.summary.message = 'No applicable schedules found. Caregiver does not meet requirements for any of the ' +
            reAssignmentResult.summary.totalSchedulesFound + ' schedule(s) during leave period.';
    } else {
        var messages = [];
        if (reAssignmentResult.summary.directlyAssigned > 0) {
            messages.push(reAssignmentResult.summary.directlyAssigned + ' open shift(s) available for direct assignment');
        }
        if (reAssignmentResult.summary.addedAsAlternative > 0) {
            messages.push('Added as alternative to ' + reAssignmentResult.summary.addedAsAlternative + ' scheduled shift(s)');
        }
        reAssignmentResult.summary.message = messages.join('; ');
    }

    return reAssignmentResult;
}   // <-- END of handleCaregiverLeaveCancellationFromSchedules

// ============================================================================
// ADD NEW FUNCTION HERE - formatLeaveReplacementData
// ============================================================================

function formatLeaveReplacementData(reassignmentResult, employeesDetails) {
    var replacementData = {
        leaveInfo: {
            caregiverName: reassignmentResult.caregiverName,
            caregiverEmployeeId: reassignmentResult.caregiverEmployeeId,
            caregiverQBID: getCaregiverQBID(reassignmentResult.caregiverName, employeesDetails),
            leavePeriod: reassignmentResult.leavePeriod,
            caregiverProfile: reassignmentResult.caregiverDetails
        },
        replacements: { data: [] },
        summary: {
            totalSchedulesInLeavePeriod: reassignmentResult.affectedSchedules.total,
            directReplacements: reassignmentResult.summary.directlyAssigned,
            alternativesAdded: reassignmentResult.summary.addedAsAlternative,
            noReplacementFound: reassignmentResult.summary.notApplicableCount
        }
    };

    function buildComparativeAnalysis(criteriaResult, action) {
        var comparison = {
            evaluation: action,
            requirements: {},
            capabilities: {},
        ...
        if (!criteriaResult) return comparison;

        if (criteriaResult.clientRequirements) {
            comparison.requirements = criteriaResult.clientRequirements;
        }
        if (criteriaResult.caregiverCapabilities) {
            comparison.capabilities = criteriaResult.caregiverCapabilities;
        }

        var checks = [];

        if (criteriaResult.hardConstraints) {
            for (var key in criteriaResult.hardConstraints) {
                if (criteriaResult.hardConstraints.hasOwnProperty(key)) {
                    var constraint = criteriaResult.hardConstraints[key];
                    checks.push({
                        type: 'HARD_CONSTRAINT',
                        name: key,
                        passed: constraint.passed,
                        clientNeeds: constraint.required || 'N/A',
                        caregiverHas: constraint.caregiverHas || 'N/A',
                        isBlocker: !constraint.passed && constraint.isHardConstraint !== false
                    });
                }
            }
        }

        if (criteriaResult.softPreferences) {
            for (var prefKey in criteriaResult.softPreferences) {
                if (criteriaResult.softPreferences.hasOwnProperty(prefKey)) {
                    var pref = criteriaResult.softPreferences[prefKey];
                    checks.push({
                        type: 'SOFT_PREFERENCE',
                        name: prefKey,
                        passed: true,
                        clientNeeds: pref.required || 'N/A',
                        caregiverHas: pref.caregiverHas || 'N/A',
                        matchInfo: pref.matchCount !== undefined ?
                            (pref.matchCount + '/' + pref.totalRequired + ' (' + pref.matchPercentage + ')') : 'N/A'
                    });
                }
            }
        }

        comparison.checkResults = checks;
        return comparison;
    }

    // Process Direct Assignments
    for (var i = 0; i < reassignmentResult.affectedSchedules.directAssignments.length; i++) {
        var assignment = reassignmentResult.affectedSchedules.directAssignments[i];
        var comparative = buildComparativeAnalysis(assignment.criteriaResult, 'DIRECT_ASSIGNMENT');

        replacementData.replacements.data.push({
            scheduleId: assignment.scheduleId,
            clientName: assignment.clientName,
            clientId: assignment.clientId,
            scheduleDate: assignment.date,
            scheduleDay: assignment.day,
            serviceTime: assignment.serviceTime,
            shiftHours: assignment.hours,
            previousStatus: assignment.previousStatus,
            newStatus: 'Scheduled',
            action: 'DIRECT_REPLACEMENT',
            replacementCaregiver: reassignmentResult.caregiverName,
            replacementCaregiverEmployeeId: reassignmentResult.caregiverEmployeeId,
            replacementCaregiverQBID: assignment.caregiverQBID,
            updateFields: {
                Expected_Caregiver: reassignmentResult.caregiverName,
                Caregiver_Id: reassignmentResult.caregiverEmployeeId,
                User_Id: assignment.caregiverQBID || 0,
                Scheduling_Status: 'Scheduled',
                Expected_Hours: assignment.hours
            },
            comparativeAnalysis: comparative,
            remarks: [
                'SUCCESS: Open shift filled',
                'Caregiver: ' + reassignmentResult.caregiverName,
    ...
    for (var j = 0; j < reassignmentResult.affectedSchedules.addedToAlternatives.length; j++) {
        var alternative = reassignmentResult.affectedSchedules.addedToAlternatives[j];
        var comparative = buildComparativeAnalysis(alternative.criteriaResult, 'ADDED_AS_ALTERNATIVE');
        ...
        if (currentList) {
            ...
            for (var k = 0; k < lines.length; k++) {
                var line = normStr(lines[k]);
                if (line) existingEntries.push(line);
            }
        }

        var newEntry = (existingEntries.length + 1) + '. ' + alternative.alternativeEntry;
        existingEntries.push(newEntry);
        var updatedList = existingEntries.join('\n');

        replacementData.replacements.data.push({
            scheduleId: alternative.scheduleId,
            clientName: alternative.clientName,
            clientId: alternative.clientId,
            scheduleDate: alternative.date,
            scheduleDay: alternative.day,
            serviceTime: alternative.serviceTime,
            previousStatus: alternative.previousStatus,
            newStatus: 'Scheduled (with alternatives)',
            action: 'ADDED_AS_ALTERNATIVE',
            currentCaregiver: alternative.currentCaregiver,
            alternativeCaregiver: reassignmentResult.caregiverName,
            updateFields: {
                List_of_Available_Caregivers: updatedList
            },
            previousAvailableList: currentList || '(empty)',
            newAvailableList: updatedList,
            comparativeAnalysis: comparative,
            remarks: [
                'ALTERNATIVE: Added to backup list',
                'Current: ' + alternative.currentCaregiver,
                'Alternative: ' + reassignmentResult.caregiverName,
                'Client: ' + alternative.clientName,
    ...
    for (var n = 0; n < reassignmentResult.affectedSchedules.notApplicable.length; n++) {
        var notApplicable = reassignmentResult.affectedSchedules.notApplicable[n];
        var comparative = buildComparativeAnalysis(notApplicable.criteriaResult, 'NOT_APPLICABLE');

        var failureReasons = ['ERROR: Requirements not met'];
        var blockingCriteria = [];

        if (comparative.checkResults) {
            for (var c = 0; c < comparative.checkResults.length; c++) {
                var check = comparative.checkResults[c];
                if (check.type === 'HARD_CONSTRAINT' && !check.passed && check.isBlocker) {
                    blockingCriteria.push({
                        criterion: check.name,
                        clientNeeds: check.clientNeeds,
                        caregiverHas: check.caregiverHas
                    });

                    failureReasons.push(
                        check.name + ' MISMATCH',
                        'Client needs: ' + check.clientNeeds,
                        'Caregiver has: ' + check.caregiverHas
                    );
                }
            }
        }
        ...
        if (blockingCriteria.length === 0) {
            failureReasons.push('Reason: ' + notApplicable.reason);
        }

        replacementData.replacements.data.push({
            scheduleId: notApplicable.scheduleId,
            clientName: notApplicable.clientName,
            clientId: notApplicable.clientId || 'N/A',
            scheduleDate: notApplicable.date,
            scheduleDay: notApplicable.day || 'N/A',
            serviceTime: notApplicable.serviceTime,
            previousStatus: notApplicable.currentStatus,
            newStatus: notApplicable.currentStatus,
            action: 'NO_REPLACEMENT_FOUND',
            replacementCaregiver: '',
            updateFields: {},
            blockingCriteria: blockingCriteria,
            comparativeAnalysis: comparative,
            remarks: failureReasons
        });
    }
...
function buildReAssignmentObject(reassignmentResult, employeesDetails) {
    var reAssignment = {
        data: []
    };

    // Get caregiver QB ID
    var caregiverQBID = getCaregiverQBID(reassignmentResult.caregiverName, employeesDetails);
    var caregiverEmployeeId = reassignmentResult.caregiverEmployeeId;

    // Check if any schedules were found
    if (reassignmentResult.affectedSchedules.total === 0) {
        return {
            reAssignment: {
                data: []
            },
            remarks: "No schedules found during leave period"
        };
    }

    // ONLY process direct assignments (open shifts that can be filled)
    // DO NOT include alternatives or not-found cases
    for (var i = 0; i < reassignmentResult.affectedSchedules.directAssignments.length; i++) {
        var assignment = reassignmentResult.affectedSchedules.directAssignments[i];

        reAssignment.data.push({
            scheduleId: assignment.scheduleId,
            employeeId: caregiverEmployeeId,
            qbId: parseInt(caregiverQBID, 10) || 0,
            remarks: "SUCCESS: Open shift filled - Caregiver: " + reassignmentResult.caregiverName +
                ", Client: " + assignment.clientName +
                ", Date: " + assignment.date +
                ", Time: " + assignment.serviceTime,
            scheduleStatus: "Scheduled Completed",
            shiftStatus: "Scheduled",
            userId: parseInt(caregiverQBID, 10) || 0,
            existingCaregivers: reassignmentResult.caregiverName,
            DFCStatus: "Directly Assigned"
        });
    }

    // Add summary message if no direct assignments found
    var summaryMessage = "";
    if (reassignmentResult.affectedSchedules.directAssignments.length === 0) {
        summaryMessage = "No open shifts found for direct assignment. " +
            "Alternatives available: " + reassignmentResult.affectedSchedules.addedToAlternatives.length + ", " +
            "Not applicable: " + reassignmentResult.affectedSchedules.notApplicable.length;
    } else {
        summaryMessage = "Successfully assigned " + reassignmentResult.affectedSchedules.directAssignments.length +
            " open shift(s) to " + reassignmentResult.caregiverName;
    }

    return {
        reAssignment: reAssignment,
        remarks: summaryMessage
    };
}

if (input.caregiveLeaveData && input.getscheduledlist) {
    var reassignmentResult = handleCaregiverLeaveCancellationFromSchedules(
        input.caregiveLeaveData,
        input.getscheduledlist,
        employeesDetails,
        leavesData,
        allClientsScheduleData,
        settingsTableData
    );

    result.caregiverLeaveReassignment = reassignmentResult;
    result.LeaveData = formatLeaveReplacementData(reassignmentResult, employeesDetails);
    result.reAssignmentData = buildReAssignmentObject(reassignmentResult, employeesDetails);  // <-- ADD THIS LINE
}
