...
function isCaregiverAlreadyAssignedToThisSchedule(caregiverName, clientId, scheduleDate, startTimeStr, endTimeStr, allSchedules) {
    ...
    for (var i = 0; i < allSchedules.length; i++) {
        ...
        var assignedCg = normName(safeGetValue(sched.fields, 'Actual_Caregiver.value', ''));
        ...
        // Check if this caregiver is already assigned to this schedule
        if (isMatchingClient && isMatchingSchedule && assignedCg === cgNorm && assignedCg !== '' && assignedCg !== 'unassigned') {
            // Additional check: make sure it's an active assignment
...
function checkCaregiverAvailabilityByType(caregiverName, dayName, startMin, endMin, employeesDetails, caregiverAvailability) {
    ...
    var availabilityTypeNorm = normStr(availabilityType).toLowerCase();

    // If explicitly set to Custom Time, use custom schedule validation
    if (availabilityTypeNorm === 'custom time' || availabilityTypeNorm === 'customtime') {
        return isCaregiverAvailableForCustomTime(caregiverName, dayName, startMin, endMin, caregiverAvailability);
    }

    // Check if employee has custom schedules even if not explicitly set
    var employeeId = getCaregiverEmployeeId(caregiverName, employeesDetails);
    if (employeeId && caregiverAvailability) {
        var availabilityData = Array.isArray(caregiverAvailability) ?
            caregiverAvailability :
            (caregiverAvailability.data || []);

        var hasCustomSchedules = false;
        for (var i = 0; i < availabilityData.length; i++) {
            var schedule = availabilityData[i];
            if (schedule) {  // ← FIX: Remove fields check since refId is at root level
        ...
        // PRIORITY CHANGE: If custom schedules exist, use custom time validation
        // This overrides time band settings when custom schedules are present
        if (hasCustomSchedules) {
            return isCaregiverAvailableForCustomTime(caregiverName, dayName, startMin, endMin, caregiverAvailability);
        }
    }

    // Default to time band (AM/PM/NOC) validation only if no custom schedules exist
    return isCaregiverAvailableForTimeBand(emp, dayName, startMin, endMin);
}
...
if (isLeaveCancellation) {
    ...
    // Get completely unassigned schedules
    var openShifts = returnAllUnassignedSchedules || [];

    // NEW: Also get partially filled multi-caregiver schedules that need more caregivers
    ...
    // Combine both types of available shifts
    ...
    detailedLogs.push("Found " + (returnAllUnassignedSchedules || []).length + " completely unassigned shifts + " + partiallyFilledSchedules.length + " partially filled multi-caregiver schedules.");
    ...
    for (var j = 0; j < openShifts.length; j++) {
        ...
        if (unassignedDate >= leaveStartDate && unassignedDate <= leaveEndDate) {
            ...
            var unassignedClientName = clientName;
            var checkClientId = unassignedClientId || unassignedClientName;
            var unassignedStartTimeStr = safeGetValue(unassignedSchedule.fields, 'Schedule_Start_Time.value', '');
            var unassignedEndTimeStr = safeGetValue(unassignedSchedule.fields, 'Schedule_End_Time.value', '');

            if (checkClientId && isCaregiverAlreadyAssignedToThisSchedule(
                caregiverOnLeave,
                checkClientId,
                unassignedDate,
                unassignedStartTimeStr,
                unassignedEndTimeStr,
                getassignedCaregiverSchedules.concat(returnAllScheduledCompletedassignedSchedules || [])
            )) {
                detailedLogs.push("--- Skipping Open Shift " + unassignedSchedule.id + " - " + caregiverOnLeave + " is already assigned to another slot of this multi-caregiver schedule.");
                continue;
            }

            // NEW: Check if this is a ghost shift and if caregiver is eligible
            ...
            if (isUnassignedGhostShift) {
                if (!caregiverEmpRecord) {
                    detailedLogs.push("--- Skipping Ghost Shift " + unassignedSchedule.id + " - caregiver record not found ---");
                    continue;
                }
                ...
                if (!(isEligible === 'yes' || isEligible === 'true')) {
                    detailedLogs.push("--- Skipping Ghost Shift " + unassignedSchedule.id + " - " + caregiverOnLeave + " is not eligible for ghost shifts (LastMinute_Ready_Ghost_Pool is not 'Yes') ---");
                    continue;
                }
            }
    ...
    for (var i = 0; i < prioritizedSchedules.length; i++) {
        ...
        for (var k = 0; k < employeesDetails.length; k++) {
            ...
            // NEW: Skip if this caregiver is already assigned to another slot of the same multi-caregiver schedule
            var scheduleClientId = safeGetValue(schedule.fields, 'Client_Id.value', '');
            var scheduleClientName = clientName; // clientName already extracted earlier
            ...
            if (checkClientId && isCaregiverAlreadyAssignedToThisSchedule(
                ...
                scheduleEndTimeStr,
                getassignedCaregiverSchedules
            )) {
                candidateLogs.push("--- Skipping: " + caregiverName + " (already assigned to another slot of this multi-caregiver schedule) ---");
                continue;
            }
