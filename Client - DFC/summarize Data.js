...
// Get JobCode_Id from cleintCreated
var jobCodeId = cleintCreated && cleintCreated.results && cleintCreated.results.jobcodes && cleintCreated.results.jobcodes["1"] && cleintCreated.results.jobcodes["1"].id
    ? cleintCreated.results.jobcodes["1"].id
    : 0;

// Build client lookup for single client
var clientLookup = {};
if (clientData && clientData.id) {
    clientLookup[clientData.id] = {
        id: clientData.id || "",
        name: (clientData.fields.Client_Full_Name ? clientData.fields.Client_Full_Name.value : "") || "",
        gender: (clientData.fields.Gender ? clientData.fields.Gender.value : "") || "",
        slotCode: (clientData.fields.Slot_Code ? clientData.fields.Slot_Code.value : "") || "",
        clientId: (clientData.fields.ID ? clientData.fields.ID.value : "") || "",
        primaryCaregiver: (clientData.fields.Primary_Caregiver ? clientData.fields.Primary_Caregiver.value : "") || "",
        secondaryCaregiver: (clientData.fields.Secondary_Caregiver ? clientData.fields.Secondary_Caregiver.value : "") || "",
        tertiaryCaregiver: (clientData.fields.Tertiary_Caregiver ? clientData.fields.Tertiary_Caregiver.value : "") || "",
        defaultServiceType: (clientData.fields.Default_Service_Type ? clientData.fields.Default_Service_Type.value : "") || "",
        jobCodeId: jobCodeId // <-- Use jobCodeId from cleintCreated
    };
}
...
function calculateHours(startTime, endTime) {
    ...
    // Special case: 00:00 to 23:59 should be 24.0 hours
    if (startMinutes === 0 && endMinutes === 1439) return 24.0;
...
// NEW FUNCTION: Generate formatted list of available caregivers
function generateAvailableCaregiversList(caregivers) {
    ...
    for (var i = 0; i < caregivers.length; i++) {
        var caregiver = caregivers[i];
        result += (i + 1) + ". " + caregiver.caregiverName + " - " + caregiver.weightedTotalScore + "\n";
    }
...
summary = {
    ...
    // NEW: Caregiver scores with detailed statistics
    caregiverScores: [],
    // Debug information
...
for (var i = 0; i < assignCaregiverData.allClientAssignments.length; i++) {
    ...
    for (var j = 0; j < client.scheduledServices.length; j++) {
        ...
        // NEW: Generate available caregivers list
        var availableCaregivers = "";
        if (Array.isArray(service.availableCaregiversList) && service.availableCaregiversList.length > 0) {
            availableCaregivers = generateAvailableCaregiversList(service.availableCaregiversList);
        }
        ...
        summary.allSchedules.push({
            ...
            caregiverScheduledHours: caregiverScheduledHours || 0,
            clientRequestedHours: clientRequestedHours || 0,
            hoursDeltaScheduledMinusCalculated: hoursDeltaScheduledMinusCalculated || 0,
            hoursDeltaRequestedMinusCalculated: hoursDeltaRequestedMinusCalculated || 0,
            hoursDeltaScheduledMinusRequested: hoursDeltaScheduledMinusRequested || 0,
            // NEW: numeric availability issues count per schedule
            scheduleavailabilityissues: scheduleAvailabilityIssues,
            // NEW: List of available caregivers
            list_of_available_caregivers: availableCaregivers,
            // NEW: Assigned caregiver's weighted score
            assignedCaregiverWeightedScore: service.weightedScore || 0,
            scheduleType: service.scheduleType || "Single Caregiver Schedule",
            caregiversRequired: service.caregiversRequired || 1,
            caregiverSlotNumber: service.caregiverSlotNumber || 1,

            shiftStatus: service.shiftStatus || "",
            scheduleStatus: scheduleStatus || "",
            conflictsCount: service.conflictsCount || 0,
            availabilityIssue: service.availabilityIssue || "",
            jobCodeId: Number(resolvedJobCodeId) || 0,
            ...
            clientDetails: {
                ...
                slotCode: clientDetails.slotCode || "", // <-- Added line
...
// Process ghost shifts and add to allSchedules
if (Array.isArray(assignCaregiverData.ghostShifts)) {
    for (var g = 0; g < assignCaregiverData.ghostShifts.length; g++) {
        var ghostShift = assignCaregiverData.ghostShifts[g];

        // Calculate hours for ghost shift
        var ghostHours = 0;
        if (typeof ghostShift.startTime === 'number' && typeof ghostShift.endTime === 'number') {
            ghostHours = (ghostShift.endTime - ghostShift.startTime) / 60; // Convert minutes to hours
        }

        // Format service time string
        var ghostServiceTime = (ghostShift.startTimeStr || "00:00") + " - " + (ghostShift.endTimeStr || "00:00");

        // Determine schedule status for ghost shift
        var ghostScheduleStatus = "";
        if (ghostShift.caregiverName) {
            ghostScheduleStatus = "Ghost Shift - Assigned";
        } else {
            ghostScheduleStatus = "Ghost Shift - Unassigned";
        }

        summary.allSchedules.push({
            clientId: "",
            clientName: "",
            caregiverName: ghostShift.caregiverName || "",
            caregiverQBID: ghostShift.caregiverQBID || 0,
            caregiverEmployeeId: ghostShift.caregiverEmployeeId || "",
            startDate: ghostShift.date || null,
            endDate: ghostShift.date || null,
            day: ghostShift.day || "",
            serviceTime: ghostServiceTime,
            startTime: ghostShift.startTimeStr || "",
            endTime: ghostShift.endTimeStr || "",
            hours: ghostHours || 0,
            caregiverScheduledHours: ghostHours || 0,
            clientRequestedHours: 0,
            hoursDeltaScheduledMinusCalculated: 0,
            hoursDeltaRequestedMinusCalculated: 0,
            hoursDeltaScheduledMinusRequested: 0,
            scheduleavailabilityissues: 0,
            // No available caregivers list for ghost shifts
            list_of_available_caregivers: "",
            // No assigned caregiver weighted score for ghost shifts
            assignedCaregiverWeightedScore: 0,
            scheduleType: "Single Caregiver Schedule",
            caregiversRequired: 1,
            caregiverSlotNumber: 1,
            shiftStatus: "Ghost Shift",
            scheduleStatus: ghostScheduleStatus,
            conflictsCount: 0,
            availabilityIssue: "",
            jobCodeId: 0,
            isGhostShift: true,
            ghostShiftId: ghostShift.id || "",
            clientDetails: {
                id: "",
                name: "",
                gender: "",
...
// Generate caregiver scores - add after all schedules and ghost shifts are processed
function generateCaregiverScores(schedules) {
    var caregiverStats = {};

    // Process all schedules to collect caregiver statistics
    for (var i = 0; i < schedules.length; i++) {
        var schedule = schedules[i];
        var caregiverName = schedule.caregiverName;

        // Skip empty caregivers
        if (!caregiverName) {
            continue;
        }
        ...
        if (!caregiverStats[caregiverName]) {
            caregiverStats[caregiverName] = {
                name: caregiverName,
                employeeId: schedule.caregiverEmployeeId || "",
                quickbooksId: schedule.caregiverQBID || 0,
                totalHours: 0,
                totalShifts: 0,
                ghostShifts: 0,
                ghostShiftHours: 0,
        ...
        var stats = caregiverStats[caregiverName];
        var hours = schedule.hours || 0;
        ...
        // Track day worked
        if (schedule.day) {
            stats.daysWorked[schedule.date] = true;
            stats.shiftsByDay[schedule.day]++;
        }

        // Differentiate between ghost shifts and client shifts
        if (schedule.isGhostShift) {
            stats.ghostShifts++;
            stats.ghostShiftHours += hours;
        } else {
            stats.clientShifts++;
            stats.clientShiftHours += hours;

            // Track unique clients served
            if (schedule.clientId) {
                stats.clientsServed[schedule.clientId] = true;
            }
        }
    }
    ...
    var caregiverScores = [];
    for (var name in caregiverStats) {
        if (caregiverStats.hasOwnProperty(name)) {
            var caregiver = caregiverStats[name];

            // Count unique clients and days worked
            caregiver.uniqueClientCount = Object.keys(caregiver.clientsServed).length;
            caregiver.uniqueDaysWorked = Object.keys(caregiver.daysWorked).length;

            // Calculate utilization score (example: weighted combination of shifts and hours)
            caregiver.utilizationScore = (caregiver.clientShiftHours * 2) +
                (caregiver.ghostShiftHours * 1) +
                (caregiver.uniqueClientCount * 3);

            caregiverScores.push(caregiver);
        }
    }

    // Sort by utilization score (highest first)
    caregiverScores.sort(function (a, b) {
...
// Generate and add caregiver scores to summary
summary.caregiverScores = generateCaregiverScores(summary.allSchedules);
...
(function buildApiCalendarPayload() {
    ...
    for (var i = 0; i < summary.allSchedules.length; i++) {
        ...
        // Create unique key - add isGhostShift to make ghost shifts unique
        var keyParts = [s.clientId || "ghost", s.startDate, s.startTime, s.endDate, s.endTime, s.caregiverName || ''];
        if (s.isGhostShift) {
            keyParts.push('ghost');
            keyParts.push(s.ghostShiftId || '');
        }
        ...
        var assignedUserId = Number(s.caregiverQBID) || 0;

        // Normalize job code id (keep as string if non-numeric)
        var jobcodeIdVal = Number(s.jobCodeId) || 0;
        var parsedJc = parseInt(jobcodeIdVal, 10);
        if (!isNaN(parsedJc)) jobcodeIdVal = parsedJc;

        // Special title for ghost shifts
        var title = s.shiftStatus || 'Scheduled';
        if (s.isGhostShift) {
            title = s.caregiverName ? 'Ghost - Assigned' : 'Ghost - Unassigned';
        }
        ...
        events.push({
            ...
            assigned_user_ids: assignedUserId ? String(assignedUserId) : "",
            jobcode_id: jobcodeIdVal ? String(jobcodeIdVal) : 0,
            title: title || "",
            draft: true,
            active: true,
            // is_ghost_shift: s.isGhostShift || false
        });
    }
...
summary.qbClientPayload = {
    data: [
        {
            name: clientData.fields.Client_Full_Name.value || "",
            parent_id: 0,
            active: true,
            short_code: "",
            billable: false,
